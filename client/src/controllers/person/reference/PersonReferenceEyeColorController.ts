...
export async function PersonReferenceControllerEyeColorIndex(pagination: TypePaginationForm): Promise<ModelCommonReferencePaginationResponse> {
    try {
        ...
        console.error("Error fetching eye color reference:", error);
        return {
            pagination: {
                search: "",
                sort_by: "",
...
export async function PersonReferenceControllerEyeColorUpsert(data: TypeInputEntityReferenceForm): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully saved eye color reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            ...
            returned.message = responseData.message || responseData.brief || "Failed to save eye color reference.";
            if (responseData.errors) returned.errors = responseData.errors;
            return returned;
        }

        returned.message = isUpdate ? "Successfully updated eye color reference." : "Successfully created eye color reference.";
        return returned;
    } catch (error: any) {
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while saving eye color reference.";
        return returned;
    }
}

export async function PersonReferenceControllerEyeColorDelete(data: TypeInputEntityReferenceForm | { id?: string | null }): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully deleted eye color reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            returned.is_error = true;
            returned.code = response.status;
            returned.message = responseData.message || responseData.brief || "Failed to delete eye color reference.";
            return returned;
        }
        ...
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while deleting eye color reference.";
        return returned;
    }
}

export async function PersonReferenceControllerEyeColorList(): Promise<{
    code: number;
    ...
    try {
        ...
        if (!response.ok) {
            return {
                ...
                message: "Failed to fetch eye color list",
            };
        }
