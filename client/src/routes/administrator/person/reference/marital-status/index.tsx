...
import type { PersonReferenceMaritalStatus } from '~/models/person/reference/MaritalStatus';
...
export default function PersonReferenceMaritalStatusPage() {
    const [items, setItems] = createSignal<PersonReferenceMaritalStatus[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<PersonReferenceMaritalStatus | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await PersonReferenceControllerMaritalStatusIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as PersonReferenceMaritalStatus[];
            ...
            console.error('Error loading marital status data:', error);
            toast.danger('Failed to load marital status data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: PersonReferenceMaritalStatus) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: PersonReferenceMaritalStatus) => {
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerMaritalStatusUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Marital status created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create marital status.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerMaritalStatusUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Marital status updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update marital status.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await PersonReferenceControllerMaritalStatusDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Marital status deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete marital status.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for legal and demographic marital statuses.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by status name (e.g. Belum Kawin, Kawin, Cerai Hidup)..."
                            onInput={handleSearch}
                            id="input-search-marital-status"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-marital-status"
                        >
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No marital status records found</p>
                                                        <p class="text-xs">Click "Add Marital Status" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                        ...
                        <Show
                            ...
                            fallback={
                                <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                    No marital status records found.
                                </div>
                            }
                        >
                            <For each={items()}>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new marital status reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Belum Kawin, Kawin, Cerai..."
                                value={formData().name}
                    ...
                    <div class="mt-3 border border-neutral-200 dark:border-neutral-700 overflow-hidden">
                        <table class="w-full text-sm text-left">
                            <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                ...
                                <tr class="bg-neutral-50/50 dark:bg-neutral-800/30">
                                    <td class="px-3 py-2 font-medium text-neutral-500 dark:text-neutral-400">Marital Status Name</td>
