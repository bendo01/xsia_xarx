...
const TASK_NAME: &str = "EstimateGetSemester";
const API_ACTION: &str = "GetSemester";
...
pub struct EstimateGetSemester;

impl EstimateGetSemester {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetSemesterResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_semester = record
            .id_semester
            .clone()
            .ok_or_else(|| DbErr::Custom("id_semester is missing".to_string()))?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetSemesterResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetSemester {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetSemester data from Feeder Dikti"
    }
