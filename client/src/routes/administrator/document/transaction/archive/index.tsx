...
import {
    DocumentArchiveControllerIndex,
    DocumentArchiveControllerUpsert,
    DocumentArchiveControllerDelete,
} from '~/controllers/document/DocumentArchiveController';
import { DocumentArchiveTypeControllerList } from '~/controllers/document/DocumentArchiveTypeController';

export default function DocumentTransactionArchivePage() {
    const [items, setItems] = createSignal<DocumentArchive[]>([]);
    ...
    const [archiveTypeOptions, setArchiveTypeOptions] = createSignal<ModelSelectItem[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<DocumentArchive | null>(null);

    const loadOptions = async () => {
        try {
            const res = await DocumentArchiveTypeControllerList();
            if (Array.isArray(res.message)) setArchiveTypeOptions(res.message);
        } catch (e) {
            console.error('Error loading archive type options:', e);
        }
    };
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await DocumentArchiveControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as DocumentArchive[];
            ...
            toast.danger('Failed to load archive data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const getArchiveTypeName = (id?: string | null) => {
        if (!id) return '-';
        const found = archiveTypeOptions().find((opt) => opt.id === id || opt.value === id);
    ...
    const openCreateModal = () => {
        setFormData({
            ...
            mimetype: 'application/pdf',
            size: null,
            archive_type_id: archiveTypeOptions()[0]?.id || '',
            description: '',
            is_knowledge: false,
            archiveable_type: '',
            archiveable_id: '',
    ...
    const openEditModal = (item: DocumentArchive) => {
        setFormData({
            id: item.id,
            name: item.name || '',
            dir: item.dir || '',
            ...
            is_knowledge: Boolean(item.is_knowledge),
    ...
    const openDeleteModal = (item: DocumentArchive) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Archive title/filename is required.';
        }

        if (!currentForm.dir || currentForm.dir.trim() === '') {
            errors.dir = 'Directory path is required.';
        }

        if (!currentForm.mimetype || currentForm.mimetype.trim() === '') {
            errors.mimetype = 'MIME type is required (e.g. application/pdf).';
        }

        if (!currentForm.archive_type_id || currentForm.archive_type_id.trim() === '') {
            errors.archive_type_id = 'Archive type is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await DocumentArchiveControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Archive created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create archive.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await DocumentArchiveControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Archive updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update archive.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await DocumentArchiveControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Archive deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete archive.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage uploaded document archives, digital assets, and system knowledge attachments.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by document / archive name..."
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Document Name</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Archive Type</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">MIME Type</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Size</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Knowledge Base</th>
                                        ...
                                        <For each={Array.from({ length: 3 })}>
                                            {() => (
                                                <tr class="animate-pulse hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                                                    ...
                                                    <td class="px-6 py-4"><div class="h-4 w-16 bg-neutral-200 dark:bg-neutral-700"></div></td>
                                                    <td class="px-6 py-4"><div class="h-4 w-16 bg-neutral-200 dark:bg-neutral-700"></div></td>
                                            ...
                                            <tr>
                                                <td colspan="6" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No archive records found</p>
                                                        <p class="text-xs">Click "Add Archive" to record a new document file.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                                                ...
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white flex items-center gap-2">
                                                            <svg class="size-4 text-neutral-400 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                                            </svg>
                                                            <span>{item.name}</span>
                                                        </div>
                                                        <div class="text-xs text-neutral-500 dark:text-neutral-400 font-mono pl-6">{item.dir}</div>
                                                    </td>
                                                    <td class="px-6 py-4 text-xs text-neutral-600 dark:text-neutral-300">
                                                        <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                                                            {getArchiveTypeName(item.archive_type_id)}
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4 text-xs font-mono text-neutral-600 dark:text-neutral-400">
                                                        {item.mimetype}
                                                    </td>
                                                    <td class="px-6 py-4 text-xs font-mono text-neutral-600 dark:text-neutral-400">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <Show
                                                            when={item.is_knowledge}
                                                            fallback={
                                                                <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-700">
                                                                    No
                                                                </span>
                                                            }
                                                        >
                                ...
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-neutral-500 dark:text-neutral-400 font-mono truncate">{item.dir}</p>
                                        </div>
                                        ...
                                        <div class="flex justify-between items-center pt-2 border-t border-neutral-100 dark:border-neutral-800">
                                            <span class="text-xs">
                                                {item.is_knowledge ? 'Knowledge Base' : ''}
                                            </span>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Register a new document archive attachment.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Document / File Name <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                ...
                                placeholder="e.g. Ijazah_S1_2023.pdf"
                                value={formData().name}
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Archive Type <span class="text-red-500">*</span>
                            </label>
                            <select
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().archive_type_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                value={formData().archive_type_id}
                                onChange={(e) => setFormData({ ...formData(), archive_type_id: e.currentTarget.value })}
                            >
                                <option value="">Select Archive Type</option>
                                <For each={archiveTypeOptions()}>
                                    {(opt) => <option value={opt.id}>{opt.label}</option>}
                                </For>
                            </select>
                            {formErrors().archive_type_id && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().archive_type_id}</p>
                            )}
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                    Directory / Path <span class="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().dir ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                    placeholder="/uploads/documents"
                                    value={formData().dir}
                                    onInput={(e) => setFormData({ ...formData(), dir: e.currentTarget.value })}
                                />
                                {formErrors().dir && (
                                    <p class="text-xs text-red-500 mt-1">{formErrors().dir}</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().mimetype ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                    placeholder="application/pdf"
                                    value={formData().mimetype}
                                    onInput={(e) => setFormData({ ...formData(), mimetype: e.currentTarget.value })}
                                />
                                {formErrors().mimetype && (
                                    <p class="text-xs text-red-500 mt-1">{formErrors().mimetype}</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 pt-2">
                            <input
                                type="checkbox"
                                id="create-is-knowledge"
                                class="size-4 text-blue-600 border-neutral-300 rounded-none focus:ring-blue-500"
                                checked={formData().is_knowledge}
                                onChange={(e) => setFormData({ ...formData(), is_knowledge: e.currentTarget.checked })}
                            />
                            <label for="create-is-knowledge" class="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            >
                                {isSubmitting() ? 'Creating...' : 'Save Archive'}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify document archive metadata.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleEditSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center gap-2 pt-2">
                            <input
                                type="checkbox"
                                id="edit-is-knowledge"
                                class="size-4 text-blue-600 border-neutral-300 rounded-none focus:ring-blue-500"
                                checked={formData().is_knowledge}
                                onChange={(e) => setFormData({ ...formData(), is_knowledge: e.currentTarget.checked })}
                            />
                            <label for="edit-is-knowledge" class="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                Include in Knowledge Base
                            </label>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            <button
                                ...
                                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-none transition-colors cursor-pointer"
                            >
                                {isSubmitting() ? 'Updating...' : 'Update Archive'}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.name} ({selectedItem()?.dir})
                    </p>
