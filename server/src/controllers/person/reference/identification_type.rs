...
#[endpoint(tags("Person Reference - IdentificationType"), status_codes(200, 400, 404, 500))]
pub async fn get_identification_type(
    req: &mut Request,
    depot: &mut Depot,
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .map_err(|e| StatusError::internal_server_error().brief(e.to_string()))?
        .ok_or_else(|| {
            StatusError::not_found().brief(format!("IdentificationType not found"))
        })?;
...
#[endpoint(tags("Person Reference - IdentificationType"), status_codes(200, 400, 500))]
pub async fn create_identification_type(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<ReferenceResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
...
pub async fn delete_identification_type(
    ...
    Ok(Json(MessageResponse {
        message: format!("IdentificationType deleted successfully"),
    }))
}
