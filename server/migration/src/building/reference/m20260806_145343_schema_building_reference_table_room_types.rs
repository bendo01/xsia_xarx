...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_145343_schema_building_reference_table_room_types"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS building_reference")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("building_reference"), Alias::new("room_types")))
                    .if_not_exists()
                    .col(
