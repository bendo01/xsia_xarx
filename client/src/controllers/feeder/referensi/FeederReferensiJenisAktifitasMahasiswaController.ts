...
const path = 'feeder/referensi/jenis-aktifitas-mahasiswa';
...
export async function FeederReferensiControllerJenisAktifitasMahasiswaIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenisAktifitasMahasiswa>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenisAktifitasMahasiswaIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenisAktifitasMahasiswaUpsert(
    form: {
        id?: string | null;
        id_jenis_aktivitas_mahasiswa?: number | null;
        nama_jenis_aktivitas_mahasiswa?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenisAktifitasMahasiswa }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenis_aktivitas_mahasiswa: Number(form.id_jenis_aktivitas_mahasiswa),
            nama_jenis_aktivitas_mahasiswa: form.nama_jenis_aktivitas_mahasiswa,
        };
...
export async function FeederReferensiControllerJenisAktifitasMahasiswaDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenisAktifitasMahasiswaList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jenis_aktivitas_mahasiswa || String(item.id),
        }));
        return {
            code: 200,
            message: items,
