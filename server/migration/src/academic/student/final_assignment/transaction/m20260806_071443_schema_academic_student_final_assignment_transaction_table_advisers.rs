...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_071443_schema_academic_student_final_assignment_transaction_table_advisers"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_student_final_assignment_transaction"), Alias::new("advisers")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("thread").integer().not_null(),
                    )
                    .col(
                        ColumnDef::new("lecturer_id").uuid().not_null(),
                    )
                    .col(
                    ...
                    .col(
                        ColumnDef::new("adviser_category_id").uuid().not_null(),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("academic_student_final_assignment_transaction_advisers_pkey")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
