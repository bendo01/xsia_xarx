...
const path = 'feeder/referensi/ikatan-kerja-sumber-daya-manusia';
...
export async function FeederReferensiControllerIkatanKerjaSumberDayaManusiaIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiIkatanKerjaSDM>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerIkatanKerjaSumberDayaManusiaIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerIkatanKerjaSumberDayaManusiaUpsert(
    form: {
        id?: string | null;
        id_ikatan_kerja?: string | null;
        nama_ikatan_kerja?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiIkatanKerjaSDM }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_ikatan_kerja: form.id_ikatan_kerja,
            nama_ikatan_kerja: form.nama_ikatan_kerja,
        };
...
export async function FeederReferensiControllerIkatanKerjaSumberDayaManusiaDelete(
    props: { id: string },
...
export async function FeederReferensiControllerIkatanKerjaSumberDayaManusiaList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_ikatan_kerja || String(item.id),
        }));
        return {
            code: 200,
            message: items,
