...
const path = 'feeder/referensi/periode-lampau';
...
export async function FeederReferensiControllerPeriodeLampauIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiPeriodeLampau>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerPeriodeLampauIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerPeriodeLampauUpsert(
    form: {
        id?: string | null;
        semester?: string | null;
        program_studi?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiPeriodeLampau }> {
    try {
...
export async function FeederReferensiControllerPeriodeLampauDelete(
    props: { id: string },
...
export async function FeederReferensiControllerPeriodeLampauList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.program_studi || String(item.id),
        }));
        return {
            code: 200,
            message: items,
