...
import type { FeederReferensiPangkatGolongan } from '~/models/feeder/referensi/PangkatGolongan';
import {
    FeederReferensiControllerPangkatGolonganIndex,
    FeederReferensiControllerPangkatGolonganUpsert,
    FeederReferensiControllerPangkatGolonganDelete,
} from '~/controllers/feeder/referensi/FeederReferensiPangkatGolonganController';

export default function FeederReferensiPangkatGolonganPage() {
    const [items, setItems] = createSignal<FeederReferensiPangkatGolongan[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiPangkatGolongan | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerPangkatGolonganIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiPangkatGolongan[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_pangkat || '').toLowerCase();
                    const bVal = String(b.nama_pangkat || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi pangkat & golongan from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiPangkatGolongan) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiPangkatGolongan) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_pangkat || currentForm.nama_pangkat.trim() === '') {
            errors.nama_pangkat = 'Nama Pangkat is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerPangkatGolonganUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerPangkatGolonganDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi jenjang kepangkatan dan ruang golongan aparatur PDDIKTI Feeder.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            id="input-search-pangkat-golongan"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-pangkat-golongan"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-pangkat-golongan"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Pangkat Golongan</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Pangkat</th>
                                                ...
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    <td class="px-6 py-4 font-mono">
                                                        <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-neutral-800 dark:text-neutral-200">
                                                            {item.id_pangkat_golongan ?? '-'}
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_pangkat}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_pangkat}</h3>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Referensi Pangkat & Golongan
                            </h2>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. 1"
                                value={formData().id_pangkat_golongan}
                                onInput={(e) => setFormData({ ...formData(), id_pangkat_golongan: e.currentTarget.value })}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_pangkat ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_pangkat}
                                onInput={(e) => setFormData({ ...formData(), nama_pangkat: e.currentTarget.value })}
                            />
                            {formErrors().nama_pangkat && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_pangkat}</p>
                            )}
                        </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Referensi Pangkat & Golongan
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
