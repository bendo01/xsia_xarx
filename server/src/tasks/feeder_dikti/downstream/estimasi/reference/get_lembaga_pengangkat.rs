...
const TASK_NAME: &str = "EstimateGetLembagaPengangkat";
const API_ACTION: &str = "GetLembagaPengangkat";
...
pub struct EstimateGetLembagaPengangkat;

impl EstimateGetLembagaPengangkat {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetLembagaPengangkatResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_lembaga_angkat = record
            .id_lembaga_angkat
            .clone()
            .ok_or_else(|| DbErr::Custom("id_lembaga_angkat is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetLembagaPengangkatResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetLembagaPengangkatResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetLembagaPengangkat {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetLembagaPengangkat data from Feeder Dikti"
    }
