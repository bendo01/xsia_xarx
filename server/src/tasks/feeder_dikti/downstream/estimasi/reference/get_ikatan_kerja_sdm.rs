...
const TASK_NAME: &str = "EstimateGetIkatanKerjaSdm";
const API_ACTION: &str = "GetIkatanKerjaSdm";
...
pub struct EstimateGetIkatanKerjaSdm;

impl EstimateGetIkatanKerjaSdm {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetIkatanKerjaSdmResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_ikatan_kerja = record
            .id_ikatan_kerja
            .clone()
            .ok_or("id_ikatan_kerja is missing")?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetIkatanKerjaSdmResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetIkatanKerjaSdmResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetIkatanKerjaSdm {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetIkatanKerjaSdm data from Feeder Dikti"
    }
