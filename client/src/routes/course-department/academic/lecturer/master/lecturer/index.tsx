...
export default function CourseDepartmentLecturerMasterPage() {
    const [searchParams, setSearchParams] = useSearchParams();

    // Data states
    const [lecturers, setLecturers] = createSignal<any[]>([]);
    ...
    const loadReferences = async () => {
        try {
            ...
            if (resolvedUnit) {
                const matchingUnit = uList.find((u: any) => u.id === resolvedUnit) || staffResult()?.unit;
                if (matchingUnit) {
    ...
    // Load academic years specific to lecturers enrolled in the given unit_id or institution_id
    

    // Main fetch lecturers query based on unit_id, institution_id, and filters
    const fetchLecturers = async () => {
        ...
        try {
            ...
            const res = await masterApiIndex('academic/lecturer/transaction/homebases', {
                page: page(),
                page_size: pageSize(),
    ...
    // Reactive effect: fetch lecturers whenever unit_id, filters, page or pageSize change
    createEffect(() => {
            ...
            <main class="flex-1 w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="relative">
                                <input
                                    type="text"
                                    placeholder="Search by NIDN / code..."
                                    value={searchCode()}
                        ...
                        <div class="space-y-1">
                            ...
                            >
                                <option value="">All Lecturer Statuses</option>
                                <For each={statuses()}>
                                    {(st) => (
                                        <option value={st.id}>
                ...
                <div class="bg-white dark:bg-neutral-800 rounded-xs border border-neutral-200 dark:border-neutral-700 shadow-2xs overflow-hidden">
                    ...
                    <div class="p-4 sm:p-5 border-b border-neutral-200 dark:border-neutral-700 flex flex-col sm:flex-row items-center justify-between gap-3">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <div class="flex items-center gap-1.5">
                                ...
                                >
                                    <option value="code-asc">NIDN / Code (Ascending)</option>
                                    <option value="code-desc">NIDN / Code (Descending)</option>
                                    <option value="name-asc">Full Name (A-Z)</option>
                                    <option value="name-desc">Full Name (Z-A)</option>
                                    <option value="registered-desc">Academic Rank (Newest)</option>
                                    <option value="registered-asc">Academic Rank (Oldest)</option>
                                </select>
                            </div>
                    ...
                    <Show
                        ...
                        fallback={
                            <Loader
                                message="Loading department lecturers from server..."
                                color="teal"
                                size="lg"
                                class="py-20"
                            />
                    ...
                    >
                                    ...
                                    <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                        <div class="flex flex-col items-center justify-center gap-2 max-w-xs mx-auto">
                                            ...
                                            <p class="text-sm font-bold text-neutral-700 dark:text-neutral-200">
                                                No lecturer records found
                                            </p>
                                            <p class="text-xs text-neutral-400">
                                                {hasActiveFilters()
                                                    ? 'No lecturers match the current search filters.'
                                                    : `There are currently no admitted lecturer records in ${activeUnitData()?.name || 'this department'}.`}
                                            </p>
                                            <Show when={hasActiveFilters()}>
                                                <button
                                                    type="button"
                            ...
                            >
                                {(std) => (
                                    <div class="p-4 space-y-3 hover:bg-neutral-50/70 dark:hover:bg-neutral-700/20 transition-colors">
                                        {/* Card Header: Avatar, Name, NIDN & Status */}
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="flex items-start gap-3 min-w-0">
                                                <div class="size-9 rounded-xs bg-gradient-to-br from-teal-500 to-cyan-600 text-white font-bold text-xs flex items-center justify-center shrink-0 shadow-2xs mt-0.5">
                                                    {(std.lecturer_name || '-' || 'S').slice(0, 1).toUpperCase()}
                                                </div>
                                        ...
                                        <div class="bg-neutral-50/80 dark:bg-neutral-900/40 rounded-xs p-2.5 space-y-1.5 border border-neutral-100 dark:border-neutral-700/60 text-xs">
                                            ...
                                            <div class="flex items-center justify-between gap-2">
                                                <span class="text-neutral-500 dark:text-neutral-400">Contract Type:</span>
                                                <span class="font-semibold text-neutral-700 dark:text-neutral-300 text-end">
                                                    {std.contract_name || '-'}
                                                </span>
                                            </div>
                                            <div class="flex items-center justify-between gap-2">
                                                <span class="text-neutral-500 dark:text-neutral-400">Academic Rank:</span>
                                                <span class="font-mono text-neutral-600 dark:text-neutral-400 text-end">
                                                    {std.lecturer?.rank_name || '-'}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex items-center justify-end pt-1">
                                            <A
                                                href={`/course-department/academic/lecturer/master/lecturer/${std.id}`}
                            ...
                            <table class="w-full text-xs text-start">
                                <thead class="bg-neutral-50/80 dark:bg-neutral-900/60 text-neutral-500 font-mono uppercase text-[10px] border-b border-neutral-200 dark:border-neutral-700">
                                    <tr>
                                        ...
                                        <th class="py-3.5 px-4 text-start">Lecturer Group</th>
                                        <th class="py-3.5 px-4 text-start">Contract Type</th>
                                            ...
                                            <tr>
                                                <td colspan="8" class="py-16 text-center">
                                                    <div class="flex flex-col items-center justify-center gap-2 text-neutral-400 dark:text-neutral-500 max-w-md mx-auto">
                                                        ...
                                                        <p class="text-xs text-neutral-400">
                                                            {hasActiveFilters()
                                                                ? 'No lecturers match the current search filters. Try clearing some filters or searching for different terms.'
                                                                : `There are currently no admitted lecturer records in ${activeUnitData()?.name || 'this department'}.`}
                                                        </p>
                                                        <Show when={hasActiveFilters()}>
                                                            <button
                        ...
                        <div class="p-4 border-t border-neutral-200 dark:border-neutral-700 flex flex-col sm:flex-row items-center justify-between gap-3 bg-neutral-50/50 dark:bg-neutral-900/30">
                            <span class="text-xs text-neutral-500 dark:text-neutral-400 font-mono">
                                Showing page <strong class="text-neutral-800 dark:text-neutral-200">{page()}</strong> of <strong class="text-neutral-800 dark:text-neutral-200">{totalPages()}</strong> ({totalItems()} total lecturers)
                            </span>
