...
const path = 'feeder/referensi/jenis-pendaftaran';
...
export async function FeederReferensiControllerJenisPendaftaranIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenisPendaftaran>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenisPendaftaranIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenisPendaftaranUpsert(
    form: {
        id?: string | null;
        id_jenis_daftar?: number | null;
        nama_jenis_daftar?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenisPendaftaran }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenis_daftar: Number(form.id_jenis_daftar),
            nama_jenis_daftar: form.nama_jenis_daftar,
        };
...
export async function FeederReferensiControllerJenisPendaftaranDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenisPendaftaranList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jenis_daftar || String(item.id),
        }));
        return {
            code: 200,
            message: items,
