...
const TASK_NAME: &str = "EstimateGetPenghasilan";
const API_ACTION: &str = "GetPenghasilan";
...
pub struct EstimateGetPenghasilan;

impl EstimateGetPenghasilan {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetPenghasilanResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_penghasilan = record
            .id_penghasilan
            .ok_or_else(|| DbErr::Custom("id_penghasilan is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetPenghasilanResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetPenghasilanResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetPenghasilan {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetPenghasilan data from Feeder Dikti"
    }
