...
use crate::dtos::feeder::referensi::kategori_kegiatan::{
    CreateKategoriKegiatanRequest, KategoriKegiatanQuery, KategoriKegiatanResponse, PaginatedKategoriKegiatanResponse,
    UpdateKategoriKegiatanRequest,
};
use crate::dtos::common::reference::MessageResponse;
use crate::models::feeder::referensi::kategori_kegiatan as entity_mod;
...
pub async fn list_kategori_kegiatan(
    ...
    let query: KategoriKegiatanQuery = req.parse_queries().unwrap_or_default();
    ...
    let data = items.into_iter().map(|item| KategoriKegiatanResponse {
            id: item.id,
    ...
    Ok(Json(PaginatedKategoriKegiatanResponse {
        data,
        total,
        page,
        page_size,
...
pub async fn get_kategori_kegiatan(
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .ok_or_else(|| StatusError::not_found().brief("KategoriKegiatan not found"))?;

    Ok(Json(KategoriKegiatanResponse {
            id: item.id,
            id_kategori_kegiatan: item.id_kategori_kegiatan,
            nama_kategori_kegiatan: item.nama_kategori_kegiatan,
            created_at: item.created_at,
...
pub async fn create_kategori_kegiatan(
        ...
        let payload: CreateKategoriKegiatanRequest = req.parse_json().await.map_err(|e| {
...
pub async fn update_kategori_kegiatan(
        ...
        let payload: UpdateKategoriKegiatanRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
...
pub async fn delete_kategori_kegiatan(
        ...
        Ok(Json(MessageResponse {
            message: "KategoriKegiatan deleted successfully".to_string(),
        }))
}
