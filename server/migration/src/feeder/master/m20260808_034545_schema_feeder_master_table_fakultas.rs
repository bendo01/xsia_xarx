...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260808_034545_schema_feeder_master_table_fakultas"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("feeder_master"), Alias::new("fakultas")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("id_fakultas").uuid(),
                    )
                    .col(
                        ColumnDef::new("nama_fakultas").string_len(255),
                    )
                    .col(
                        ColumnDef::new("status").string_len(255),
                    )
                    .col(
                        ColumnDef::new("id_jenjang_pendidikan").uuid(),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("feeder_master_fakultas_pkey")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
