...
export async function AcademicStudentFinalAssignmentReferenceControllerStageIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicStudentFinalAssignmentReferenceStage>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/student/final-assignment/reference/stages?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicStudentFinalAssignmentReferenceControllerStageIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicStudentFinalAssignmentReferenceControllerStageUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicStudentFinalAssignmentReferenceStage }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/student/final-assignment/reference/stages/${form.id}`
            : `${getBaseUrl()}/academic/student/final-assignment/reference/stages`;
...
export async function AcademicStudentFinalAssignmentReferenceControllerStageDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/student/final-assignment/reference/stages/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
