...
export async function AcademicStudentReferenceControllerSelectionTypeIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicStudentReferenceSelectionType>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/student/reference/selection-types?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicStudentReferenceControllerSelectionTypeIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicStudentReferenceControllerSelectionTypeUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicStudentReferenceSelectionType }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/student/reference/selection-types/${form.id}`
            : `${getBaseUrl()}/academic/student/reference/selection-types`;
...
export async function AcademicStudentReferenceControllerSelectionTypeDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/student/reference/selection-types/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
