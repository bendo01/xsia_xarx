...
const path = 'feeder/referensi/pekerjaan';
...
export async function FeederReferensiControllerPekerjaanIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiPekerjaan>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerPekerjaanIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerPekerjaanUpsert(
    form: {
        id?: string | null;
        id_pekerjaan?: number | null;
        nama_pekerjaan?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiPekerjaan }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_pekerjaan: Number(form.id_pekerjaan),
...
export async function FeederReferensiControllerPekerjaanDelete(
    props: { id: string },
...
export async function FeederReferensiControllerPekerjaanList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_pekerjaan || String(item.id),
        }));
        return {
            code: 200,
            message: items,
