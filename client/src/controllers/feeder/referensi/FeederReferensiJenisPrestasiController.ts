...
const path = 'feeder/referensi/jenis-prestasi';
...
export async function FeederReferensiControllerJenisPrestasiIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenisPrestasi>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenisPrestasiIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenisPrestasiUpsert(
    form: {
        id?: string | null;
        id_jenis_prestasi?: number | null;
        nama_jenis_prestasi?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenisPrestasi }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenis_prestasi: Number(form.id_jenis_prestasi),
            nama_jenis_prestasi: form.nama_jenis_prestasi,
        };
...
export async function FeederReferensiControllerJenisPrestasiDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenisPrestasiList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jenis_prestasi || String(item.id),
        }));
        return {
            code: 200,
            message: items,
