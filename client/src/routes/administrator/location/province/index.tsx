...
import {
    LocationProvinceControllerIndex,
    LocationProvinceControllerUpsert,
    LocationProvinceControllerDelete,
} from '~/controllers/location/LocationProvinceController';
import { LocationCountryControllerList } from '~/controllers/location/LocationCountryController';

export default function LocationProvincePage() {
    const [items, setItems] = createSignal<LocationProvince[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<LocationProvince | null>(null);

    const loadOptions = async () => {
        try {
            const countriesRes = await LocationCountryControllerList();
            if (Array.isArray(countriesRes.message)) setCountryOptions(countriesRes.message);
        } catch (e) {
            console.error('Error loading country options:', e);
        }
    };
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await LocationProvinceControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as LocationProvince[];
            ...
            toast.danger('Failed to load province data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const getCountryName = (id?: string | null) => {
        if (!id) return '-';
        const found = countryOptions().find((opt) => opt.id === id || opt.value === id);
    ...
    const openCreateModal = () => {
        setFormData({
            ...
            dikti_code: '',
            epsbed_code: '',
            country_id: countryOptions()[0]?.id || '',
            description: '',
            slug: '',
        });
        setFormErrors({});
    ...
    const openEditModal = (item: LocationProvince) => {
        setFormData({
            id: item.id,
            code: item.code || '',
            name: item.name || '',
    ...
    const openDeleteModal = (item: LocationProvince) => {
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationProvinceControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Province created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create province.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationProvinceControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Province updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update province.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await LocationProvinceControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Province deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete province.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage first-level administrative subdivisions and provincial territories.
                    </p>
                </div>
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Province Name</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Country</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No province records found</p>
                                                        <p class="text-xs">Click "Add Province" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new province location record.
                            </p>
                        </div>
                        <button
                            type="button"
