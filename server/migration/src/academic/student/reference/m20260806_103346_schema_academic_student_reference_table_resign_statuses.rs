...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_103346_schema_academic_student_reference_table_resign_statuses"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_student_reference")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_student_reference"), Alias::new("resign_statuses")))
                    .if_not_exists()
                    .col(
