...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            "
            CREATE SCHEMA IF NOT EXISTS academic_lecturer_master;
            ",
        )
        .await?;
        db.execute_unprepared(
            "
            CREATE TABLE IF NOT EXISTS academic_lecturer_master.lecturers
            (
            id uuid NOT NULL DEFAULT gen_random_uuid(),
                ...
                CONSTRAINT alm_lecturers_pkey PRIMARY KEY (id)
            )
            ",
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("DROP TABLE IF EXISTS academic_lecturer_master.lecturers")
            .await?;
