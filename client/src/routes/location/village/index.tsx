...
export default function LocationVillagePage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await LocationVillageControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as LocationVillage[];
            ...
            toast.danger('Failed to load village data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openCreateModal = () => {
        setFormData({
            ...
            code: '',
            name: '',
            sub_district_id: subDistrictOptions()[0]?.id || '',
            dikti_code: '',
            slug: '',
            alt_slug: '',
        });
    ...
    const openEditModal = (item: LocationVillage) => {
        setFormData({
            id: item.id,
            code: item.code || '',
            name: item.name || '',
    ...
    const openDeleteModal = (item: LocationVillage) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.code || currentForm.code.trim() === '') {
            errors.code = 'Village code is required.';
        }
        ...
        if (!currentForm.sub_district_id || currentForm.sub_district_id.trim() === '') {
            errors.sub_district_id = 'Sub-district / Kecamatan is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationVillageControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Village created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create village.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationVillageControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Village updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update village.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await LocationVillageControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Village deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete village.', 5000);
            }
        } catch (error: any) {
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by village / kelurahan name (e.g. Dago, Sadang Serang)..."
                            onInput={handleSearch}
                            id="input-search-location-village"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-location-village"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-location-village"
                        >
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No village records found</p>
                                                        <p class="text-xs">Click "Add Village" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new village / kelurahan location record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Village Code <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Sub-District / Kecamatan <span class="text-red-500">*</span>
                            </label>
                            <select
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().sub_district_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                value={formData().sub_district_id}
                                onChange={(e) => setFormData({ ...formData(), sub_district_id: e.currentTarget.value })}
                            >
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            >
                                {isSubmitting() ? 'Creating...' : 'Save Village'}
                            </button>
                        </div>
                    </form>
                </div>
                    ...
                    <form onSubmit={handleEditSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            <button
                                ...
                                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-none transition-colors cursor-pointer"
                            >
                                {isSubmitting() ? 'Updating...' : 'Update Village'}
                            </button>
                        </div>
                    </form>
                </div>
