...
use crate::dtos::feeder::referensi::agama::{
    CreateAgamaRequest, AgamaQuery, AgamaResponse, PaginatedAgamaResponse,
    UpdateAgamaRequest,
};
use crate::dtos::common::reference::MessageResponse;
use crate::models::feeder::referensi::agama as entity_mod;
...
pub async fn list_agama(
    ...
    let query: AgamaQuery = req.parse_queries().unwrap_or_default();
    ...
    let data = items.into_iter().map(|item| AgamaResponse {
            id: item.id,
    ...
    Ok(Json(PaginatedAgamaResponse {
        data,
        total,
        page,
        page_size,
...
pub async fn get_agama(
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .ok_or_else(|| StatusError::not_found().brief("Agama not found"))?;

    Ok(Json(AgamaResponse {
            id: item.id,
            id_agama: item.id_agama,
            nama_agama: item.nama_agama,
            created_at: item.created_at,
...
pub async fn create_agama(
        ...
        let payload: CreateAgamaRequest = req.parse_json().await.map_err(|e| {
...
pub async fn update_agama(
        ...
        let payload: UpdateAgamaRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
...
pub async fn delete_agama(
        ...
        Ok(Json(MessageResponse {
            message: "Agama deleted successfully".to_string(),
        }))
}
