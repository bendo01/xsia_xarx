...
export default function LocationRegencyPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await LocationRegencyControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as LocationRegency[];
            ...
            console.error('Error loading regency data:', error);
            toast.danger('Failed to load regency data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openCreateModal = () => {
        setFormData({
            ...
            code: '',
            name: '',
            province_id: provinceOptions()[0]?.id || '',
            regency_type_id: regencyTypeOptions()[0]?.id || '',
            dikti_code: '',
            epsbed_code: '',
            description: '',
            slug: '',
    ...
    const openEditModal = (item: LocationRegency) => {
        setFormData({
            id: item.id,
            code: item.code || '',
            name: item.name || '',
    ...
    const openDeleteModal = (item: LocationRegency) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Regency name is required.';
        }

        if (!currentForm.province_id || currentForm.province_id.trim() === '') {
            errors.province_id = 'Province is required.';
        }

        if (!currentForm.regency_type_id || currentForm.regency_type_id.trim() === '') {
            errors.regency_type_id = 'Regency type is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationRegencyControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Regency created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create regency.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationRegencyControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Regency updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update regency.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await LocationRegencyControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Regency deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete regency.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                ...
                <div class="mt-4 sm:mt-0 flex items-center gap-2 justify-end">
                    <button
                        ...
                        id="btn-add-location-regency"
                    >
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M5 12h14" />
                            <path d="M12 5v14" />
                        </svg>
                        <span>Add Regency</span>
                    </button>
                </div>
            </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-location-regency"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-location-regency"
                        >
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No regency records found</p>
                                                        <p class="text-xs">Click "Add Regency" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Regency Name <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().province_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                    value={formData().province_id}
                                    onChange={(e) => setFormData({ ...formData(), province_id: e.currentTarget.value })}
                                >
                                    <option value="">Select Province</option>
                                    <For each={provinceOptions()}>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            >
                                {isSubmitting() ? 'Creating...' : 'Save Regency'}
                            </button>
                        </div>
                    </form>
                </div>
                    ...
                    <form onSubmit={handleEditSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            <button
                                ...
                                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-none transition-colors cursor-pointer"
                            >
                                {isSubmitting() ? 'Updating...' : 'Update Regency'}
                            </button>
                        </div>
                    </form>
                </div>
