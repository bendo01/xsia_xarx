...
const TASK_NAME: &str = "EstimateRiwayatFungsionalDosen";
const API_ACTION: &str = "GetRiwayatFungsionalDosen";
...
pub struct EstimateRiwayatFungsionalDosen;

impl EstimateRiwayatFungsionalDosen {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInput) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let id_jabatan_fungsional = record.id_jabatan_fungsional.ok_or_else(|| DbErr::Custom("id_jabatan_fungsional is required for upsert".to_string()))?;
...
#[async_trait]
impl Task for EstimateRiwayatFungsionalDosen {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetRiwayatFungsionalDosen data from Feeder Dikti"
    }
