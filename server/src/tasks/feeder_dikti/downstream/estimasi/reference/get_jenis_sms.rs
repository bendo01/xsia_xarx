...
const TASK_NAME: &str = "EstimateGetJenisSMS";
const API_ACTION: &str = "GetJenisSMS";
...
pub struct EstimateGetJenisSMS;

impl EstimateGetJenisSMS {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisSMSResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenis_sms = record
            .id_jenis_sms
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenis_sms is missing".to_string()))?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenisSMSResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenisSMS {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenisSMS data from Feeder Dikti"
    }
