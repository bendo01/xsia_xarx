...
export async function AcademicCourseReferenceControllerCurriculumTypeIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCourseReferenceCurriculumType>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/curriculum-types?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCourseReferenceControllerCurriculumTypeIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCourseReferenceControllerCurriculumTypeUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCourseReferenceCurriculumType }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/course/reference/curriculum-types/${form.id}`
            : `${getBaseUrl()}/academic/course/reference/curriculum-types`;
...
export async function AcademicCourseReferenceControllerCurriculumTypeDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/curriculum-types/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
