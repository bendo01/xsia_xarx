...
const TASK_NAME: &str = "EstimateGetJenisPrestasi";
const API_ACTION: &str = "GetJenisPrestasi";
...
pub struct EstimateGetJenisPrestasi;

impl EstimateGetJenisPrestasi {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisPrestasiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenis_prestasi = record
            .id_jenis_prestasi
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenis_prestasi is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJenisPrestasiResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenisPrestasiResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenisPrestasi {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenisPrestasi data from Feeder Dikti"
    }
