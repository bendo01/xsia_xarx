...
const path = "person/reference/hair-color";

export async function PersonReferenceControllerHairColorIndex(pagination: TypePaginationForm): Promise<ModelCommonReferencePaginationResponse> {
    try {
        ...
        console.error("Error fetching hair color reference:", error);
        return {
            pagination: {
                search: "",
                sort_by: "",
...
export async function PersonReferenceControllerHairColorUpsert(data: TypeInputEntityReferenceForm): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully saved hair color reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            ...
            returned.message = responseData.message || responseData.brief || "Failed to save hair color reference.";
            if (responseData.errors) returned.errors = responseData.errors;
            return returned;
        }

        returned.message = isUpdate ? "Successfully updated hair color reference." : "Successfully created hair color reference.";
        return returned;
    } catch (error: any) {
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while saving hair color reference.";
        return returned;
    }
}

export async function PersonReferenceControllerHairColorDelete(data: TypeInputEntityReferenceForm | { id?: string | null }): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully deleted hair color reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            returned.is_error = true;
            returned.code = response.status;
            returned.message = responseData.message || responseData.brief || "Failed to delete hair color reference.";
            return returned;
        }
        ...
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while deleting hair color reference.";
        return returned;
    }
}

export async function PersonReferenceControllerHairColorList(): Promise<{
    code: number;
    ...
    try {
        ...
        if (!response.ok) {
            return {
                ...
                message: "Failed to fetch hair color list",
            };
        }
