...
import type { AcademicCampaignReferenceCalendarCategory } from '~/models/academic/campaign/reference/CalendarCategory';
import type { TypeInputEntityReferenceForm } from '~/lib/types';
import {
    AcademicCampaignReferenceControllerCalendarCategoryIndex,
    AcademicCampaignReferenceControllerCalendarCategoryUpsert,
    AcademicCampaignReferenceControllerCalendarCategoryDelete,
} from '~/controllers/academic/campaign/reference/AcademicCampaignReferenceCalendarCategoryController';

export default function AcademicCampaignReferenceCalendarCategoryPage() {
    const [items, setItems] = createSignal<AcademicCampaignReferenceCalendarCategory[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<AcademicCampaignReferenceCalendarCategory | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await AcademicCampaignReferenceControllerCalendarCategoryIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as AcademicCampaignReferenceCalendarCategory[];
            ...
            console.error('Error loading calendar category data:', error);
            toast.danger('Failed to load calendar category data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: AcademicCampaignReferenceCalendarCategory) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: AcademicCampaignReferenceCalendarCategory) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Calendar category name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicCampaignReferenceControllerCalendarCategoryUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Calendar category reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create calendar category reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicCampaignReferenceControllerCalendarCategoryUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Calendar category reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update calendar category reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await AcademicCampaignReferenceControllerCalendarCategoryDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Calendar category reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete calendar category reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Calendar Category / Kategori Kalender Akademik
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for academic milestones and scheduled periods.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by calendar category name (e.g. Registrasi, UTS, UAS)..."
                            onInput={handleSearch}
                            id="input-search-calendar-category"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-calendar-category"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-calendar-category"
                        >
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No calendar category records found</p>
                        ...
                        <Show
                            ...
                            fallback={
                                <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                    No calendar category records found.
                                </div>
                            }
                        >
                            <For each={items()}>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Calendar Category Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new calendar category reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Masa Registrasi"
                                value={formData().name}
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Calendar Category Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify calendar category reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="flex items-center gap-3 pb-3 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">
                                This calendar category record will be removed.
                            </p>
                        </div>
                    </div>
