...
import type { FeederReferensiStatusKepegawaian } from '~/models/feeder/referensi/StatusKepegawaian';
import {
    FeederReferensiControllerStatusKepegawaianIndex,
    FeederReferensiControllerStatusKepegawaianUpsert,
    FeederReferensiControllerStatusKepegawaianDelete,
} from '~/controllers/feeder/referensi/FeederReferensiStatusKepegawaianController';

export default function FeederReferensiStatusKepegawaianPage() {
    const [items, setItems] = createSignal<FeederReferensiStatusKepegawaian[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiStatusKepegawaian | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerStatusKepegawaianIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiStatusKepegawaian[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_status_pegawai || '').toLowerCase();
                    const bVal = String(b.nama_status_pegawai || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi status kepegawaian from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiStatusKepegawaian) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiStatusKepegawaian) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_status_pegawai || currentForm.nama_status_pegawai.trim() === '') {
            errors.nama_status_pegawai = 'Nama Status Pegawai is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerStatusKepegawaianUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerStatusKepegawaianDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi status formasi kepegawaian (PNS, PPPK, Tetap, Kontrak).
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            id="input-search-status-kepegawaian"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-status-kepegawaian"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-status-kepegawaian"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Status Pegawai</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Status Pegawai</th>
                                                ...
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    <td class="px-6 py-4 font-mono">
                                                        <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-neutral-800 dark:text-neutral-200">
                                                            {item.id_status_pegawai ?? '-'}
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_status_pegawai}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_status_pegawai}</h3>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Referensi Status Kepegawaian
                            </h2>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. 1"
                                value={formData().id_status_pegawai}
                                onInput={(e) => setFormData({ ...formData(), id_status_pegawai: e.currentTarget.value })}
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Nama Status Pegawai <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_status_pegawai ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_status_pegawai}
                                onInput={(e) => setFormData({ ...formData(), nama_status_pegawai: e.currentTarget.value })}
                            />
                            {formErrors().nama_status_pegawai && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_status_pegawai}</p>
                            )}
                        </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Referensi Status Kepegawaian
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.nama_status_pegawai}
                    </p>
