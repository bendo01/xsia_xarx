...
const path = 'feeder/referensi/negara';
...
export async function FeederReferensiControllerNegaraIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiNegara>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerNegaraIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerNegaraUpsert(
    form: {
        id?: string | null;
        id_negara?: string | null;
        nama_negara?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiNegara }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_negara: form.id_negara,
            nama_negara: form.nama_negara,
        };
...
export async function FeederReferensiControllerNegaraDelete(
    props: { id: string },
...
export async function FeederReferensiControllerNegaraList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_negara || String(item.id),
        }));
        return {
            code: 200,
            message: items,
