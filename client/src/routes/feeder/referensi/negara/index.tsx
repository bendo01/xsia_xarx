...
import type { FeederReferensiNegara } from '~/models/feeder/referensi/Negara';
import {
    FeederReferensiControllerNegaraIndex,
    FeederReferensiControllerNegaraUpsert,
    FeederReferensiControllerNegaraDelete,
} from '~/controllers/feeder/referensi/FeederReferensiNegaraController';

export default function FeederReferensiNegaraPage() {
    const [items, setItems] = createSignal<FeederReferensiNegara[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiNegara | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerNegaraIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiNegara[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_negara || '').toLowerCase();
                    const bVal = String(b.nama_negara || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi negara from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiNegara) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiNegara) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_negara || currentForm.nama_negara.trim() === '') {
            errors.nama_negara = 'Nama Negara is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerNegaraUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerNegaraDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi kode dan nama negara internasional PDDIKTI Feeder.
                    </p>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Kode / ID Negara</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Negara</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_negara}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_negara}</h3>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                onInput={(e) => setFormData({ ...formData(), id_negara: e.currentTarget.value })}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_negara ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_negara}
                                onInput={(e) => setFormData({ ...formData(), nama_negara: e.currentTarget.value })}
                            />
                            {formErrors().nama_negara && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_negara}</p>
                            )}
                        </div>
