...
const path = "person/reference/marital_status";
...
export async function PersonReferenceControllerMaritalStatusDelete(data: TypeInputEntityReferenceForm): Promise<UpsertDeleteMessage> {
    let returned = {
        is_error: false,
        code: 200,
        message: "success to upsert reference.",
...
export async function PersonReferenceControllerMaritalStatusList(): Promise<{
    ...
    try {
        const response = await fetch(`${server_api_url}person/reference/marital_statuses/list`, {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
            return {
                ...
                message: "Gagal Mengambil Data Status Perkawinan"
            };
        }
