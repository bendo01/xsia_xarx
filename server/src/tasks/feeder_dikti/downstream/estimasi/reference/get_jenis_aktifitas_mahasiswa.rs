...
const TASK_NAME: &str = "EstimateGetJenisAktifitasMahasiswa";
const API_ACTION: &str = "GetJenisAktivitasMahasiswa";
...
pub struct EstimateGetJenisAktifitasMahasiswa;

impl EstimateGetJenisAktifitasMahasiswa {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisAktifitasMahasiswaResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenis_aktivitas_mahasiswa = record
            .id_jenis_aktivitas_mahasiswa
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenis_aktivitas_mahasiswa is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJenisAktifitasMahasiswaResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenisAktifitasMahasiswaResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenisAktifitasMahasiswa {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenisAktivitasMahasiswa data from Feeder Dikti"
    }
