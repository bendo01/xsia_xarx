...
const TASK_NAME: &str = "EstimateGetPekerjaan";
const API_ACTION: &str = "GetPekerjaan";
...
pub struct EstimateGetPekerjaan;

impl EstimateGetPekerjaan {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetPekerjaanResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_pekerjaan = record
            .id_pekerjaan
            .ok_or_else(|| DbErr::Custom("id_pekerjaan is missing".to_string()))?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetPekerjaanResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetPekerjaan {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetPekerjaan data from Feeder Dikti"
    }
