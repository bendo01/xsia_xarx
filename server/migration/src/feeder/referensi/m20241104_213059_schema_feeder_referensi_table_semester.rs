...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            "
            CREATE TABLE IF NOT EXISTS feeder_referensi.semester
            (
                ...
                CONSTRAINT feeder_referensi_semester_pkey PRIMARY KEY (id)
            )
            ",
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("DROP TABLE IF EXISTS feeder_referensi.semester")
            .await?;
