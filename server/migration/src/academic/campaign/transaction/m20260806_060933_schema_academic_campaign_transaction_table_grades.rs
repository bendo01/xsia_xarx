...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_060933_schema_academic_campaign_transaction_table_grades"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_campaign_transaction"), Alias::new("grades")))
                    .if_not_exists()
                    .col(
                    ...
                    .col(
                        ColumnDef::new("grade").double().not_null().default(Expr::cust("'0'::double precision")),
                    )
                    .col(
                        ColumnDef::new("minimum").double().not_null().default(Expr::cust("'0'::double precision")),
                    )
                    .col(
                        ColumnDef::new("maximum").double().not_null().default(Expr::cust("'0'::double precision")),
                    )
                    .col(
