...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260808_034545_schema_feeder_akun_table_kredential"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS feeder_akun")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("feeder_akun"), Alias::new("kredential")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("username").string_len(255).not_null(),
                    )
                    .col(
                        ColumnDef::new("password").string_len(255).not_null(),
                    )
                    .col(
                        ColumnDef::new("institution_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("service_url").text().not_null(),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("feeder_akun_kredential_pkey")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
