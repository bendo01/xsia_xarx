...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_064613_schema_academic_general_reference_table_academic_years"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_general_reference")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_general_reference"), Alias::new("academic_years")))
                    .if_not_exists()
                    .col(
                    ...
                    .col(
                        ColumnDef::new("year").integer().not_null(),
                    )
                    .col(
                        ColumnDef::new("name").string_len(255).not_null(),
                    )
                    .col(
                        ColumnDef::new("feeder_name").string_len(255).not_null(),
                    )
                    .col(
                        ColumnDef::new("academic_year_category_id").uuid().not_null().default(Expr::cust("'00000000-0000-0000-0000-000000000000'::uuid")),
                    )
                    .col(
                    ...
                    .col(
                        ColumnDef::new("is_active").boolean().default(Expr::cust("true")),
                    )
                    .col(
