...
const path = "contact/reference/phone-types";

export async function ContactReferenceControllerPhoneTypeIndex(pagination: TypePaginationForm): Promise<ModelCommonReferencePaginationResponse> {
    try {
        ...
        console.error("Error fetching phone type reference:", error);
        return {
            pagination: {
                search: "",
                sort_by: "",
...
export async function ContactReferenceControllerPhoneTypeUpsert(data: TypeInputEntityReferenceForm): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully saved phone type reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            ...
            returned.message = responseData.message || responseData.brief || "Failed to save phone type reference.";
            if (responseData.errors) returned.errors = responseData.errors;
            return returned;
        }

        returned.message = isUpdate ? "Successfully updated phone type reference." : "Successfully created phone type reference.";
        return returned;
    } catch (error: any) {
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while saving phone type reference.";
        return returned;
    }
}

export async function ContactReferenceControllerPhoneTypeDelete(data: TypeInputEntityReferenceForm | { id?: string | null }): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully deleted phone type reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            returned.is_error = true;
            returned.code = response.status;
            returned.message = responseData.message || responseData.brief || "Failed to delete phone type reference.";
            return returned;
        }
        ...
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while deleting phone type reference.";
        return returned;
    }
}

export async function ContactReferenceControllerPhoneTypeList(): Promise<{
    code: number;
    ...
    try {
        ...
        if (!response.ok) {
            return {
                ...
                message: "Failed to fetch phone type list",
            };
        }
