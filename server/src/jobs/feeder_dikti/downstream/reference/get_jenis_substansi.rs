...
pub struct WorkerArgs {
    pub records: Vec<GetJenisSubstansiResponse>,
}
...
pub async fn start_worker(
    ...
    let worker = WorkerBuilder::new("xsia-xarx:feeder_dikti:downstream:reference:get_jenis_substansi")
        .data(db)
        .backend(storage)
        .build_fn(handle_job);
...
impl Worker {
    ...
    pub async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisSubstansiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
