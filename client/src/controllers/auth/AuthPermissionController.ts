...
export async function AuthPermissionControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<Permission>> {
    try {
        ...
        console.error('Error in AuthPermissionControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AuthPermissionControllerUpsert(
    form: {
        id?: string | null;
        name: string;
        uri?: string | null;
...
): Promise<{ is_error: boolean; message: string; data?: Permission }> {
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update permission.' : 'Failed to create permission.'),
            };
        }
        ...
        return {
            is_error: true,
            message: error.message || 'Network error while saving permission.',
        };
    }
}

export async function AuthPermissionControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete permission.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Permission deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting permission.',
        };
    }
}

export async function AuthPermissionControllerList(): Promise<{
    code: number;
