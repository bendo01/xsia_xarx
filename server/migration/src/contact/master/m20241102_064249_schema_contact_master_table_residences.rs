...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            "
            CREATE TABLE IF NOT EXISTS contact_master.residences
            (
                id uuid DEFAULT uuid_generate_v7(),
                street character varying(255)  NOT NULL,
                citizens_association integer NOT NULL DEFAULT 0,
                neighborhood_association integer NOT NULL DEFAULT 0,
                ...
                CONSTRAINT cm_residences_pkey PRIMARY KEY (id)
            )
            ",
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("DROP TABLE IF EXISTS contact_master.residences")
            .await?;
