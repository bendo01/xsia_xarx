...
import type { FeederReferensiTahunAjaran } from '~/models/feeder/referensi/TahunAjaran';
import {
    FeederReferensiControllerTahunAjaranIndex,
    FeederReferensiControllerTahunAjaranUpsert,
    FeederReferensiControllerTahunAjaranDelete,
} from '~/controllers/feeder/referensi/FeederReferensiTahunAjaranController';

export default function FeederReferensiTahunAjaranPage() {
    const [items, setItems] = createSignal<FeederReferensiTahunAjaran[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiTahunAjaran | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerTahunAjaranIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiTahunAjaran[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_tahun_ajaran || '').toLowerCase();
                    const bVal = String(b.nama_tahun_ajaran || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi tahun ajaran from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiTahunAjaran) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiTahunAjaran) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_tahun_ajaran || currentForm.nama_tahun_ajaran.trim() === '') {
            errors.nama_tahun_ajaran = 'Nama Tahun Ajaran is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerTahunAjaranUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerTahunAjaranDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi kurun tahun akademik PDDIKTI Feeder.
                    </p>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-tahun-ajaran"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Tahun Ajaran</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Tahun Ajaran</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_tahun_ajaran}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_tahun_ajaran}</h3>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                onInput={(e) => setFormData({ ...formData(), id_tahun_ajaran: e.currentTarget.value })}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_tahun_ajaran ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_tahun_ajaran}
                                onInput={(e) => setFormData({ ...formData(), nama_tahun_ajaran: e.currentTarget.value })}
                            />
                            {formErrors().nama_tahun_ajaran && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_tahun_ajaran}</p>
                            )}
                        </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.nama_tahun_ajaran}
                    </p>
