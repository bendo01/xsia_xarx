...
import {
    ...
    InstitutionMasterInstitutionControllerUpdate,
...
export default function InstitutionMasterInstitutionEditPage() {
    ...
    onMount(async () => {
        ...
        if (!id) {
            toast.danger('Missing institution ID.');
        ...
        setIsLoading(true);
        try {
            const [varieties, categories, countries, parents, academicYears, institutionRes] = await Promise.all([
                fetchInstitutionVarietyOptions(),
                fetchInstitutionCategoryOptions(),
                fetchCountryOptions(),
                InstitutionMasterInstitutionControllerList(),
            ...
            // Filter out current institution from parent options to prevent self-referencing
            setParentOptions(parents.filter((p) => p.id !== id));
            setAcademicYearOptions(academicYears);

            if (institutionRes.data?.institution) {
                const inst = institutionRes.data.institution;
                setFormData({
                    name: inst.name || '',
                    code: inst.code || '',
                    alphabet_code: inst.alphabet_code || '',
                    is_active: Boolean(inst.is_active),
                    variety_id: inst.variety_id || '',
                    category_id: inst.category_id || '',
                    country_id: inst.country_id || '',
                    parent_id: inst.parent_id && inst.parent_id !== '00000000-0000-0000-0000-000000000000' ? inst.parent_id : '',
                    feeder_id: inst.feeder_id && inst.feeder_id !== '00000000-0000-0000-0000-000000000000' ? inst.feeder_id : '',
                    academic_year_id: inst.academic_year_id && inst.academic_year_id !== '00000000-0000-0000-0000-000000000000' ? inst.academic_year_id : '',
                });
            } else {
                toast.danger('Institution record not found.');
            }
        } catch (err) {
            console.error('Failed to load institution edit data:', err);
            toast.danger('Failed to load institution data.');
        } finally {
            setIsLoading(false);
        }
    });
    ...
    const handleSubmit = async (e: Event) => {
        ...
        if (!validateForm()) {
            toast.danger('Please fill out all required fields.');
            return;
        }
        ...
        try {
            ...
            const res = await InstitutionMasterInstitutionControllerUpdate(institutionId(), payload);

            if (!res.is_error) {
                toast.success(res.message || 'Institution updated successfully!');
                setTimeout(() => {
                    window.location.href = `/institution/master/institution/show?id=${institutionId()}`;
                }, 1000);
            } else {
                toast.danger(res.message || 'Failed to update institution record.');
                if (res.errors) {
            ...
            console.error('Error submitting edit institution form:', err);
            toast.danger(err.message || 'Network error occurred while updating.');
        } finally {
            ...
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
                ...
                <div class="border-b border-neutral-200 dark:border-neutral-800 pb-4">
                    <nav class="flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400 mb-1">
                        ...
                        <span>/</span>
                        <span class="font-medium text-neutral-900 dark:text-white">Edit Record</span>
                    </nav>
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400 mt-0.5">
                                Modify institution properties, affiliations, and operational statuses.
                            </p>
                        </div>
                        <a
                            href={`/institution/master/institution/show?id=${institutionId()}`}
                ...
                <Show when={isLoading()}>
                    <div class="p-8 text-center bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 space-y-2">
                        ...
                        <p class="text-xs text-neutral-500">Loading institution details...</p>
                    </div>
                </Show>
                    ...
                    <form onSubmit={handleSubmit} class="space-y-6">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-800">
                            ...
                            >
                                ...
                                <span>{isSubmitting() ? 'Updating Institution...' : 'Update Institution'}</span>
                            </button>
                        </div>
                    </form>
                </Show>
