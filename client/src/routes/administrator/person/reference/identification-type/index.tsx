...
import type { PersonReferenceIdentificationType } from '~/models/person/reference/IdentificationType';
...
export default function PersonReferenceIdentificationTypePage() {
    const [items, setItems] = createSignal<PersonReferenceIdentificationType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<PersonReferenceIdentificationType | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await PersonReferenceControllerIdentificationTypeIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as PersonReferenceIdentificationType[];
            ...
            console.error('Error loading identification type data:', error);
            toast.danger('Failed to load identification type data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: PersonReferenceIdentificationType) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: PersonReferenceIdentificationType) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Identification type name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerIdentificationTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Identification type created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create identification type.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerIdentificationTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Identification type updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update identification type.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await PersonReferenceControllerIdentificationTypeDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Identification type deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete identification type.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Identification Type / Jenis Identitas
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for identification documents (KTP, Passport, SIM, KITAS).
                    </p>
                </div>
                <div class="mt-4 sm:mt-0 flex items-center gap-2 justify-end">
                    ...
                    >
                        ...
                        <span>Add Identification Type</span>
                    </button>
                </div>
            </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by identification type (e.g. KTP, Paspor, SIM)..."
                            onInput={handleSearch}
                            id="input-search-id-type"
                        />
                    </div>
                </div>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No identification types found</p>
                                                        <p class="text-xs">Click "Add Identification Type" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new identity document type record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. KTP, SIM, PASPOR..."
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Kartu Tanda Penduduk, Paspor..."
                                value={formData().name}
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify identity document type details.
                            </p>
                        </div>
                        <button
                            type="button"
                        ...
                        <table class="w-full text-sm text-left">
                            <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-2 text-neutral-900 dark:text-white">{selectedItem()?.alphabetic_code || (selectedItem() as any)?.alphabet_code || '-'}</td>
                                </tr>
                                <tr class="bg-neutral-50/50 dark:bg-neutral-800/30">
                                    <td class="px-3 py-2 font-medium text-neutral-500 dark:text-neutral-400">Identification Type Name</td>
