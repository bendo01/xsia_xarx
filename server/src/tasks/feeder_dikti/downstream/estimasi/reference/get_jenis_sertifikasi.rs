...
const TASK_NAME: &str = "EstimateGetJenisSertifikasi";
const API_ACTION: &str = "GetJenisSertifikasi";
...
pub struct EstimateGetJenisSertifikasi;

impl EstimateGetJenisSertifikasi {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisSertifikasiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenis_sertifikasi = record
            .id_jenis_sertifikasi
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenis_sertifikasi is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJenisSertifikasiResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenisSertifikasiResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenisSertifikasi {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenisSertifikasi data from Feeder Dikti"
    }
