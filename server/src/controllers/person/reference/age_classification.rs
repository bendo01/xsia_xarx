...
pub async fn list_age_classifications(
    req: &mut Request,
    depot: &mut Depot,
...
pub async fn get_age_classification(
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .map_err(|e| StatusError::internal_server_error().brief(e.to_string()))?
        .ok_or_else(|| {
            StatusError::not_found().brief(format!("AgeClassification not found"))
        })?;
...
pub async fn delete_age_classification(
    ...
    Ok(Json(MessageResponse {
        message: format!("AgeClassification deleted successfully"),
    }))
}
