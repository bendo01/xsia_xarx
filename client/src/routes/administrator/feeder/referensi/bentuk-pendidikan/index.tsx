...
import type { FeederReferensiBentukPendidikan } from '~/models/feeder/referensi/BentukPendidikan';
import {
    FeederReferensiControllerBentukPendidikanIndex,
    FeederReferensiControllerBentukPendidikanUpsert,
    FeederReferensiControllerBentukPendidikanDelete,
} from '~/controllers/feeder/referensi/FeederReferensiBentukPendidikanController';

export default function FeederReferensiBentukPendidikanPage() {
    const [items, setItems] = createSignal<FeederReferensiBentukPendidikan[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiBentukPendidikan | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerBentukPendidikanIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiBentukPendidikan[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_bentuk_pendidikan || '').toLowerCase();
                    const bVal = String(b.nama_bentuk_pendidikan || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi bentuk pendidikan from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiBentukPendidikan) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiBentukPendidikan) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_bentuk_pendidikan || currentForm.nama_bentuk_pendidikan.trim() === '') {
            errors.nama_bentuk_pendidikan = 'Nama Bentuk Pendidikan is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerBentukPendidikanUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerBentukPendidikanDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi bentuk satuan pendidikan PDDIKTI Feeder.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            id="input-search-bentuk-pendidikan"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-bentuk-pendidikan"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-bentuk-pendidikan"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Bentuk Pendidikan</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Bentuk Pendidikan</th>
                                                ...
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    <td class="px-6 py-4 font-mono">
                                                        <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-neutral-800 dark:text-neutral-200">
                                                            {item.id_bentuk_pendidikan ?? '-'}
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_bentuk_pendidikan}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_bentuk_pendidikan}</h3>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Referensi Bentuk Pendidikan
                            </h2>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. 1"
                                value={formData().id_bentuk_pendidikan}
                                onInput={(e) => setFormData({ ...formData(), id_bentuk_pendidikan: e.currentTarget.value })}
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Nama Bentuk Pendidikan <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_bentuk_pendidikan ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_bentuk_pendidikan}
                                onInput={(e) => setFormData({ ...formData(), nama_bentuk_pendidikan: e.currentTarget.value })}
                            />
                            {formErrors().nama_bentuk_pendidikan && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_bentuk_pendidikan}</p>
                            )}
                        </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Referensi Bentuk Pendidikan
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.nama_bentuk_pendidikan}
                    </p>
