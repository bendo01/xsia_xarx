...
const TASK_NAME: &str = "EstimateGetAllProdi";
const API_ACTION: &str = "GetAllProdi";
...
impl EstimateGetAllProdi {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetAllProdiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_prodi = record
            .id_prodi
            .ok_or_else(|| DbErr::Custom("id_prodi is missing".to_string()))?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetAllProdiResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetAllProdi {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetAllProdi data from Feeder Dikti"
    }
