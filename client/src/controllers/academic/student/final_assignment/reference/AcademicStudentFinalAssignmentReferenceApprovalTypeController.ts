...
export async function AcademicStudentFinalAssignmentReferenceControllerApprovalTypeIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicStudentFinalAssignmentReferenceApprovalType>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/student/final-assignment/reference/approval-types?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicStudentFinalAssignmentReferenceControllerApprovalTypeIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicStudentFinalAssignmentReferenceControllerApprovalTypeUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicStudentFinalAssignmentReferenceApprovalType }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/student/final-assignment/reference/approval-types/${form.id}`
            : `${getBaseUrl()}/academic/student/final-assignment/reference/approval-types`;
...
export async function AcademicStudentFinalAssignmentReferenceControllerApprovalTypeDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/student/final-assignment/reference/approval-types/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
