...
export async function AcademicCampaignReferenceControllerEncounterCategoryIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCampaignReferenceEncounterCategory>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/encounter-categories?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCampaignReferenceControllerEncounterCategoryIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCampaignReferenceControllerEncounterCategoryUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCampaignReferenceEncounterCategory }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/campaign/reference/encounter-categories/${form.id}`
            : `${getBaseUrl()}/academic/campaign/reference/encounter-categories`;
...
export async function AcademicCampaignReferenceControllerEncounterCategoryDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/encounter-categories/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
