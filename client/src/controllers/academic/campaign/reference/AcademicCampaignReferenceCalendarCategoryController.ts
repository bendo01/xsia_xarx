...
export async function AcademicCampaignReferenceControllerCalendarCategoryIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCampaignReferenceCalendarCategory>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/calendar-categories?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCampaignReferenceControllerCalendarCategoryIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCampaignReferenceControllerCalendarCategoryUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCampaignReferenceCalendarCategory }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/campaign/reference/calendar-categories/${form.id}`
            : `${getBaseUrl()}/academic/campaign/reference/calendar-categories`;
...
export async function AcademicCampaignReferenceControllerCalendarCategoryDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/calendar-categories/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
