...
use crate::dtos::institution::master::units::{
    CreateUnitRequest, UnitQuery, PaginatedUnitResponse,
    UnitResponse, UpdateUnitRequest,
};
...
pub async fn create_unit(
    ...
    let payload: CreateUnitRequest = req
        .parse_json()
        .await
...
pub async fn update_unit(
    ...
    let payload: UpdateUnitRequest = req
        .parse_json()
        .await
        .map_err(|e| StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e)))?;
    ...
    if let Some(val) = payload.unit_type_id {
        active_model.unit_type_id = Set(val);
    }
    ...
    if let Some(val) = payload.education_id {
        active_model.education_id = Set(val);
    }
    if let Some(val) = payload.feeder_id {
        active_model.feeder_id = Set(Some(val));
    }
    if let Some(val) = payload.lft {
        active_model.lft = Set(Some(val));
    }
    if let Some(val) = payload.rght {
        active_model.rght = Set(Some(val));
    }
...
fn to_response(item: entity_mod::Model) -> UnitResponse {
    UnitResponse {
        id: item.id,
        code: item.code,
        name: item.name,
