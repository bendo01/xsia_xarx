...
import type { FeederReferensiJalurMasuk } from '~/models/feeder/referensi/JalurMasuk';
import {
    FeederReferensiControllerJalurMasukIndex,
    FeederReferensiControllerJalurMasukUpsert,
    FeederReferensiControllerJalurMasukDelete,
} from '~/controllers/feeder/referensi/FeederReferensiJalurMasukController';

export default function FeederReferensiJalurMasukPage() {
    const [items, setItems] = createSignal<FeederReferensiJalurMasuk[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiJalurMasuk | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerJalurMasukIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiJalurMasuk[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_jalur_masuk || '').toLowerCase();
                    const bVal = String(b.nama_jalur_masuk || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi jalur masuk from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiJalurMasuk) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiJalurMasuk) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_jalur_masuk || currentForm.nama_jalur_masuk.trim() === '') {
            errors.nama_jalur_masuk = 'Nama Jalur Masuk is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerJalurMasukUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerJalurMasukDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi jalur penerimaan mahasiswa baru PDDIKTI Feeder.
                    </p>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-jalur-masuk"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Jalur Masuk</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Jalur Masuk</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_jalur_masuk}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_jalur_masuk}</h3>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. 1"
                                value={formData().id_jalur_masuk}
                                onInput={(e) => setFormData({ ...formData(), id_jalur_masuk: e.currentTarget.value })}
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Nama Jalur Masuk <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_jalur_masuk ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_jalur_masuk}
                                onInput={(e) => setFormData({ ...formData(), nama_jalur_masuk: e.currentTarget.value })}
                            />
                            {formErrors().nama_jalur_masuk && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_jalur_masuk}</p>
                            )}
                        </div>
