...
export async function AcademicCampaignReferenceControllerAttendTypeIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCampaignReferenceAttendType>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/attend-types?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCampaignReferenceControllerAttendTypeIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCampaignReferenceControllerAttendTypeUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCampaignReferenceAttendType }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/campaign/reference/attend-types/${form.id}`
            : `${getBaseUrl()}/academic/campaign/reference/attend-types`;
...
export async function AcademicCampaignReferenceControllerAttendTypeDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/attend-types/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
