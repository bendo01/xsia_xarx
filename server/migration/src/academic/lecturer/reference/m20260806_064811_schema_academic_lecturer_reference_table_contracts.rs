...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_064811_schema_academic_lecturer_reference_table_contracts"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_lecturer_reference")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_lecturer_reference"), Alias::new("contracts")))
                    .if_not_exists()
                    .col(
