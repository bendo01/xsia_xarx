...
import {
    AuthUserControllerIndex,
    AuthUserControllerUpsert,
    AuthUserControllerDelete,
    BackendUserResponse,
} from '~/controllers/auth/AuthUserController';
import { AuthRoleControllerList } from '~/controllers/auth/AuthRoleController';

export default function AuthUserPage() {
    const [items, setItems] = createSignal<BackendUserResponse[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<BackendUserResponse | null>(null);

    const loadOptions = async () => {
        try {
            const res = await AuthRoleControllerList();
            if (Array.isArray(res.message)) setRoleOptions(res.message);
        } catch (e) {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await AuthUserControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as BackendUserResponse[];
            ...
            toast.danger('Failed to load user accounts from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openCreateModal = () => {
        setFormData({
            ...
            password: '',
            is_active: true,
            current_role_id: roleOptions()[0]?.id || '',
        });
        setFormErrors({});
    ...
    const openEditModal = (item: BackendUserResponse) => {
        setFormData({
            id: item.id,
            name: item.name || '',
            email: item.email || '',
    ...
    const openDeleteModal = (item: BackendUserResponse) => {
    ...
    const validateForm = (isEdit = false): boolean => {
        ...
        if (!isEdit && (!currentForm.password || currentForm.password.length < 6)) {
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await AuthUserControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'User account created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create user.', 5000);
            }
        } catch (error: any) {
            toast.danger(error.message || 'Error occurred while creating user.', 5000);
        } finally {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await AuthUserControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'User account updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update user.', 5000);
            }
        } catch (error: any) {
            toast.danger(error.message || 'Error occurred while updating user.', 5000);
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await AuthUserControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'User deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete user.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        User Accounts / Manajemen Akun Pengguna
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage system operator, administrative, and stakeholder login credentials.
                    </p>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">User Details</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Assigned Role</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Status</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No user accounts found</p>
                                                        <p class="text-xs">Click "Add User" to create a new user account.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                                            ...
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    <td class="px-6 py-4">
                                                        ...
                                                        <div class="text-xs text-neutral-500 dark:text-neutral-400 font-mono mt-0.5">{item.email}</div>
                                                    </td>
                                                    <td class="px-6 py-4 text-xs text-neutral-600 dark:text-neutral-300">
                                                        <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                                                            {getRoleName(item.current_role_id)}
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        <Show
                                                            when={item.is_active}
                                                            fallback={
                                                                <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300 border border-red-200 dark:border-red-800">
                                                                    Inactive
                                                                </span>
                                                            }
                                                        >
                                ...
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div class="text-xs text-neutral-600 dark:text-neutral-300">
                                            Role: {getRoleName(item.current_role_id)}
                                        </div>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <input
                                type="email"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().email ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                            ...
                            />
                            {formErrors().email && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().email}</p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                type="password"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().password ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                            ...
                            />
                            {formErrors().password && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().password}</p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            <select
                                ...
                                value={formData().current_role_id ?? ''}
                                onChange={(e) => setFormData({ ...formData(), current_role_id: e.currentTarget.value })}
                            >
                        ...
                        <div class="flex items-center gap-2 pt-2">
                            ...
                            />
                            <label for="create-is-active" class="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                Active Account
                            </label>
                        </div>
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify user properties and status.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleEditSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center gap-2 pt-2">
                            <input
                                ...
                                onChange={(e) => setFormData({ ...formData(), is_active: e.currentTarget.checked })}
                            />
                            <label for="edit-is-active" class="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                Active Account
                            </label>
                        </div>
