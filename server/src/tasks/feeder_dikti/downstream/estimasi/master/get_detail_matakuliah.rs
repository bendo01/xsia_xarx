...
const TASK_NAME: &str = "EstimateDetailMatakuliah";
const API_ACTION: &str = "GetDetailMataKuliah";
...
pub struct EstimateDetailMatakuliah;

impl EstimateDetailMatakuliah {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputDetailMatakuliah) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let action = if let Some(existing_record) = existing {
            ...
            let new_record = matakuliah::ActiveModel {
                ...
                updated_by: Set(None),
                deleted_at: Set(None),
                nama_jenis_mata_kuliah: Set(None),
                nama_kelompok_mata_kuliah: Set(None),
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[ModelInputDetailMatakuliah],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<ModelInputDetailMatakuliah>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateDetailMatakuliah {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetDetailMataKuliah data from Feeder Dikti"
    }
