...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_070628_schema_academic_prior_learning_recognition_reference_table_evidence_categories"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_prior_learning_recognition_reference")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_prior_learning_recognition_reference"), Alias::new("evidence_categories")))
                    .if_not_exists()
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("academic_prior_learning_recognition_reference_evidence_categori")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
