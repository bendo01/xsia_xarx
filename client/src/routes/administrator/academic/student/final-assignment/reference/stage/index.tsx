...
import type { AcademicStudentFinalAssignmentReferenceStage } from '~/models/academic/student/final_assignment/reference/Stage';
import type { TypeInputEntityReferenceForm } from '~/lib/types';
import {
    AcademicStudentFinalAssignmentReferenceControllerStageIndex,
    AcademicStudentFinalAssignmentReferenceControllerStageUpsert,
    AcademicStudentFinalAssignmentReferenceControllerStageDelete,
} from '~/controllers/academic/student/final_assignment/reference/AcademicStudentFinalAssignmentReferenceStageController';

export default function AcademicStudentFinalAssignmentReferenceStagePage() {
    const [items, setItems] = createSignal<AcademicStudentFinalAssignmentReferenceStage[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<AcademicStudentFinalAssignmentReferenceStage | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await AcademicStudentFinalAssignmentReferenceControllerStageIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as AcademicStudentFinalAssignmentReferenceStage[];
            ...
            console.error('Error loading thesis stage data:', error);
            toast.danger('Failed to load thesis stage data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: AcademicStudentFinalAssignmentReferenceStage) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: AcademicStudentFinalAssignmentReferenceStage) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Thesis stage name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicStudentFinalAssignmentReferenceControllerStageUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Thesis stage reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create thesis stage reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicStudentFinalAssignmentReferenceControllerStageUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Thesis stage reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update thesis stage reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await AcademicStudentFinalAssignmentReferenceControllerStageDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Thesis stage reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete thesis stage reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Thesis Stage / Tahapan Ujian Tugas Akhir
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference final assignment exam stages (e.g. Seminar Proposal, Seminar Hasil, Sidang Skripsi).
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by stage (e.g. Seminar Proposal, Sidang)..."
                            onInput={handleSearch}
                            id="input-search-stage"
                        />
                    </div>
                </div>
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Stage Name</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No thesis stage records found</p>
                                                        <p class="text-xs">Click "Add Stage" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Thesis Stage Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new thesis exam stage reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Seminar Proposal (Sempro)"
                                value={formData().name}
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Thesis Stage Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify thesis exam stage reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="flex items-center gap-3 pb-3 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">
                                This thesis stage record will be removed.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="py-4">
                        ...
                        <table class="w-full text-xs text-left border border-neutral-200 dark:border-neutral-700">
                            <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                ...
                                <tr class="bg-neutral-50 dark:bg-neutral-800/50">
                                    <td class="px-3 py-2 font-semibold text-neutral-600 dark:text-neutral-300">Stage</td>
