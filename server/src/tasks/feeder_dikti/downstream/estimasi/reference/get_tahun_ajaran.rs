...
const TASK_NAME: &str = "EstimateGetTahunAjaran";
const API_ACTION: &str = "GetTahunAjaran";
...
pub struct EstimateGetTahunAjaran;

impl EstimateGetTahunAjaran {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetTahunAjaranResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetTahunAjaranResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetTahunAjaranResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetTahunAjaran {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetTahunAjaran data from Feeder Dikti"
    }
