...
import type { LocationRegion } from '~/models/location/Region';
import {
    LocationRegionControllerIndex,
    LocationRegionControllerUpsert,
    LocationRegionControllerDelete,
} from '~/controllers/location/LocationRegionController';

export default function LocationRegionPage() {
    const [items, setItems] = createSignal<LocationRegion[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<LocationRegion | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await LocationRegionControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as LocationRegion[];
            ...
            toast.danger('Failed to load region data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: LocationRegion) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: LocationRegion) => {
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationRegionControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Region created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create region.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationRegionControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Region updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update region.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await LocationRegionControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Region deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete region.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage geographical regions and global subcontinent groupings.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by region name (e.g. South East Asia)..."
                            onInput={handleSearch}
                            id="input-search-location-region"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-location-region"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-location-region"
                        >
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Region Name</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No region records found</p>
                                                        <p class="text-xs">Click "Add Region" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new region location entry.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            >
                                {isSubmitting() ? 'Creating...' : 'Save Region'}
                            </button>
                        </div>
                    </form>
                </div>
