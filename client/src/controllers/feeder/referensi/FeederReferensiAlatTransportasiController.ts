...
const path = 'feeder/referensi/alat-transportasi';
...
export async function FeederReferensiControllerAlatTransportasiIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiAlatTransportasi>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerAlatTransportasiIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerAlatTransportasiUpsert(
    form: {
        id?: string | null;
        id_alat_transportasi?: string | null;
        nama_alat_transportasi?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiAlatTransportasi }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_alat_transportasi: form.id_alat_transportasi,
            nama_alat_transportasi: form.nama_alat_transportasi,
        };
...
export async function FeederReferensiControllerAlatTransportasiDelete(
    props: { id: string },
...
export async function FeederReferensiControllerAlatTransportasiList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_alat_transportasi || String(item.id),
        }));
        return {
            code: 200,
            message: items,
