...
import {
    LiterateEducationControllerIndex,
    LiterateEducationControllerUpsert,
    LiterateEducationControllerDelete,
} from '~/controllers/literate/LiterateEducationController';
import { LiterateCategoryControllerList } from '~/controllers/literate/LiterateCategoryController';
import { LiterateGroupControllerList } from '~/controllers/literate/LiterateGroupController';
import { LiterateLevelControllerList } from '~/controllers/literate/LiterateLevelController';
import { LiterateVarietyControllerList } from '~/controllers/literate/LiterateVarietyController';

export default function LiterateEducationPage() {
    const [items, setItems] = createSignal<LiterateEducation[]>([]);
    ...
    const [levelOptions, setLevelOptions] = createSignal<ModelSelectItem[]>([]);
    const [groupOptions, setGroupOptions] = createSignal<ModelSelectItem[]>([]);
    ...
    const [formData, setFormData] = createSignal<TypeInputLiterateEducationForm>({
        id: null,
        code: '',
        alphabet_code: '',
        abbreviation: '',
    ...
    const [selectedItem, setSelectedItem] = createSignal<LiterateEducation | null>(null);

    const loadOptions = async () => {
        try {
            const [levelsRes, groupsRes, categoriesRes, varietiesRes] = await Promise.all([
                LiterateLevelControllerList(),
                LiterateGroupControllerList(),
                LiterateCategoryControllerList(),
                LiterateVarietyControllerList(),
            ]);

            if (Array.isArray(levelsRes.message)) setLevelOptions(levelsRes.message);
            if (Array.isArray(groupsRes.message)) setGroupOptions(groupsRes.message);
            if (Array.isArray(categoriesRes.message)) setCategoryOptions(categoriesRes.message);
            if (Array.isArray(varietiesRes.message)) setVarietyOptions(varietiesRes.message);
        } catch (error) {
            console.error('Error loading reference options for education:', error);
        }
    };
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await LiterateEducationControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as LiterateEducation[];
            ...
            console.error('Error loading literate education data:', error);
            toast.danger('Failed to load education data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const getLevelName = (id?: string) => {
        if (!id) return '-';
        const found = levelOptions().find((opt) => opt.id === id || opt.value === id);
        return found ? found.label : id;
    };

    const getGroupName = (id?: string) => {
        if (!id) return '-';
        const found = groupOptions().find((opt) => opt.id === id || opt.value === id);
        return found ? found.label : id;
    };

    const getCategoryName = (id?: string) => {
        if (!id) return '-';
        const found = categoryOptions().find((opt) => opt.id === id || opt.value === id);
        return found ? found.label : id;
    };

    const getVarietyName = (id?: string) => {
        if (!id) return '-';
        const found = varietyOptions().find((opt) => opt.id === id || opt.value === id);
        return found ? found.label : id;
    };
    ...
    const openCreateModal = () => {
        setFormData({
            ...
            abbreviation: '',
            name: '',
            level_id: levelOptions()[0]?.id || '',
            group_id: groupOptions()[0]?.id || '',
            category_id: categoryOptions()[0]?.id || '',
            variety_id: varietyOptions()[0]?.id || '',
        });
        setFormErrors({});
    ...
    const openEditModal = (item: LiterateEducation) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: LiterateEducation) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.abbreviation || currentForm.abbreviation.trim() === '') {
            errors.abbreviation = 'Abbreviation is required.';
        }

        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Education name is required.';
        }

        if (!currentForm.level_id || currentForm.level_id.trim() === '' || currentForm.level_id === '00000000-0000-0000-0000-000000000000') {
            errors.level_id = 'Level is required.';
        }

        if (!currentForm.group_id || currentForm.group_id.trim() === '' || currentForm.group_id === '00000000-0000-0000-0000-000000000000') {
            errors.group_id = 'Group is required.';
        }

        if (!currentForm.category_id || currentForm.category_id.trim() === '' || currentForm.category_id === '00000000-0000-0000-0000-000000000000') {
            errors.category_id = 'Category is required.';
        }

        if (!currentForm.variety_id || currentForm.variety_id.trim() === '' || currentForm.variety_id === '00000000-0000-0000-0000-000000000000') {
            errors.variety_id = 'Variety is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await LiterateEducationControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Education entity created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create education entity.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await LiterateEducationControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Education entity updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update education entity.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await LiterateEducationControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Education entity deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete education entity.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master education programs and degree associations across level, group, category, and variety.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by education name (e.g. Sarjana, Diploma, Magister)..."
                            onInput={handleSearch}
                            id="input-search-literate-education"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-literate-education"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-literate-education"
                        >
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-5 py-3.5 font-semibold tracking-wider w-28">Alphabet Code</th>
                                    <th scope="col" class="px-5 py-3.5 font-semibold tracking-wider w-28">Abbr</th>
                                    <th scope="col" class="px-5 py-3.5 font-semibold tracking-wider">Education Name</th>
                                    <th scope="col" class="px-5 py-3.5 font-semibold tracking-wider">Level</th>
                                    <th scope="col" class="px-5 py-3.5 font-semibold tracking-wider">Group</th>
                                    <th scope="col" class="px-5 py-3.5 font-semibold tracking-wider">Category</th>
                                    <th scope="col" class="px-5 py-3.5 font-semibold tracking-wider">Variety</th>
                                        ...
                                        <For each={Array.from({ length: 3 })}>
                                            {() => (
                                                <tr class="animate-pulse hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                                                    ...
                                                    <td class="px-5 py-4"><div class="h-5 w-16 bg-neutral-200 dark:bg-neutral-700"></div></td>
                                                    <td class="px-5 py-4"><div class="h-5 w-14 bg-neutral-200 dark:bg-neutral-700"></div></td>
                                            ...
                                            <tr>
                                                <td colspan="9" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No education records found</p>
                                                        <p class="text-xs">Click "Add Education" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-5 py-4 font-semibold text-xs text-neutral-700 dark:text-neutral-300">
                                                        {item.abbreviation || '-'}
                                                    </td>
                                                    <td class="px-5 py-4">
                                                    ...
                                                    <td class="px-5 py-4 text-xs text-neutral-600 dark:text-neutral-300">
                                                        {getVarietyName(item.variety_id)}
                                                    </td>
                                ...
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div class="text-xs text-neutral-600 dark:text-neutral-300 grid grid-cols-2 gap-1 pt-1">
                                            <div><span class="font-semibold">Level:</span> {getLevelName(item.level_id)}</div>
                                            <div><span class="font-semibold">Group:</span> {getGroupName(item.group_id)}</div>
                                            <div><span class="font-semibold">Cat:</span> {getCategoryName(item.category_id)}</div>
                                            <div><span class="font-semibold">Var:</span> {getVarietyName(item.variety_id)}</div>
                                        </div>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new education record with linked relationships.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().abbreviation ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                    placeholder="e.g. S.Kom"
                                    value={formData().abbreviation ?? ''}
                                    onInput={(e) => setFormData({ ...formData(), abbreviation: e.currentTarget.value })}
                                />
                                {formErrors().abbreviation && (
                                    <p class="text-xs text-red-500 mt-1">{formErrors().abbreviation}</p>
                                )}
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                    Education Name <span class="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().level_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                    value={formData().level_id}
                                    onChange={(e) => setFormData({ ...formData(), level_id: e.currentTarget.value })}
                                >
                                ...
                                {formErrors().level_id && (
                                    <p class="text-xs text-red-500 mt-1">{formErrors().level_id}</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <select
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().group_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                    value={formData().group_id}
                                    onChange={(e) => setFormData({ ...formData(), group_id: e.currentTarget.value })}
                                >
                                ...
                                {formErrors().group_id && (
                                    <p class="text-xs text-red-500 mt-1">{formErrors().group_id}</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().category_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                ...
                                {formErrors().category_id && (
                                    <p class="text-xs text-red-500 mt-1">{formErrors().category_id}</p>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <select
                                    class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                        formErrors().variety_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                    }`}
                                    value={formData().variety_id}
                                    onChange={(e) => setFormData({ ...formData(), variety_id: e.currentTarget.value })}
                                >
                                ...
                                {formErrors().variety_id && (
                                    <p class="text-xs text-red-500 mt-1">{formErrors().variety_id}</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            >
                                {isSubmitting() ? 'Creating...' : 'Save Education'}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Education Entity
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify education entity attributes and relationships.
                            </p>
                        </div>
                        <button
                            type="button"
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Are you sure you want to delete this education entity?
                            </p>
                        </div>
                    </div>
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.name} ({selectedItem()?.abbreviation || selectedItem()?.alphabet_code})
                    </p>
