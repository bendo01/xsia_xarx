...
use crate::models::institution::reference::unit_types as entity_mod;

#[endpoint(tags("Institution Reference - UnitTyp"), status_codes(200, 500))]
...
#[endpoint(tags("Institution Reference - UnitTyp"), status_codes(200, 400, 404, 500))]
pub async fn get_unit_types(
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .map_err(|e| StatusError::internal_server_error().brief(e.to_string()))?
        .ok_or_else(|| {
            StatusError::not_found().brief(format!("UnitTyp not found"))
        })?;
...
#[endpoint(tags("Institution Reference - UnitTyp"), status_codes(200, 400, 500))]
pub async fn create_unit_types(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<ReferenceResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
...
#[endpoint(tags("Institution Reference - UnitTyp"), status_codes(200, 400, 404, 500))]
pub async fn update_unit_types(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<ReferenceResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
...
#[endpoint(tags("Institution Reference - UnitTyp"), status_codes(200, 400, 404, 500))]
pub async fn delete_unit_types(
    req: &mut Request,
    depot: &mut Depot,
    ...
    Ok(Json(MessageResponse {
        message: format!("UnitTyp deleted successfully"),
    }))
}
