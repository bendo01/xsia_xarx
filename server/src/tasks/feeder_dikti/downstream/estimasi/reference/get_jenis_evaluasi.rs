...
use crate::models::feeder::referensi::jenis_evaluasi as jenis_evaluasi;
use crate::tasks::feeder_dikti::downstream::feeder_request::{InputRequestData, RequestData};
use crate::tasks::Task;

// Configuration constants
const TASK_NAME: &str = "EstimateGetJenisEvaluasi";
const API_ACTION: &str = "GetJenisEvaluasi";

// API Request Configuration
const DEFAULT_LIMIT: i32 = 0;
const DEFAULT_ORDER: &str = "";
const DEFAULT_FILTER: &str = "";

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct GetJenisEvaluasiResponse {
    pub id_jenis_evaluasi: Option<i32>,
    pub nama_jenis_evaluasi: String,
}

pub struct EstimateGetJenisEvaluasi;

impl EstimateGetJenisEvaluasi {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisEvaluasiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenis_evaluasi = record
            .id_jenis_evaluasi
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenis_evaluasi is missing".to_string()))?;

        let sync_time = Local::now().naive_local();

        let existing = jenis_evaluasi::Entity::find()
            .filter(jenis_evaluasi::Column::DeletedAt.is_null())
            .filter(jenis_evaluasi::Column::IdJenisEvaluasi.eq(id_jenis_evaluasi))
            .one(txn)
            .await?;

        let action = if let Some(existing_record) = existing {
            let mut active: jenis_evaluasi::ActiveModel = existing_record.into_active_model();

            // Update fields that are present in GetJenisEvaluasiResponse
            active.nama_jenis_evaluasi = Set(Some(record.nama_jenis_evaluasi.clone()));
            ...
            let new_record = jenis_evaluasi::ActiveModel {
                id: Set(pk_id),
                id_jenis_evaluasi: Set(Some(id_jenis_evaluasi)),
                nama_jenis_evaluasi: Set(Some(record.nama_jenis_evaluasi.clone())),
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJenisEvaluasiResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenisEvaluasiResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenisEvaluasi {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenisEvaluasi data from Feeder Dikti"
    }
