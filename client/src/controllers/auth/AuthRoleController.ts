...
export async function AuthRoleControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<Role>> {
    try {
        ...
        console.error('Error in AuthRoleControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
): Promise<{ is_error: boolean; message: string; data?: Role }> {
    try {
        ...
        const payload: Record<string, any> = {
            ...
            position_type_id: form.position_type_id || null,
            roleable_id: form.roleable_id || null,
            roleable_type: form.roleable_type || null,
        };
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update role.' : 'Failed to create role.'),
            };
        }
        ...
        return {
            is_error: true,
            message: error.message || 'Network error while saving role.',
        };
    }
}

export async function AuthRoleControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete role.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Role deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting role.',
        };
    }
}

export async function AuthRoleControllerList(): Promise<{
    code: number;
