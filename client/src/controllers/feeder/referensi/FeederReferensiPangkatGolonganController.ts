...
const path = 'feeder/referensi/pangkat-golongan';
...
export async function FeederReferensiControllerPangkatGolonganIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiPangkatGolongan>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerPangkatGolonganIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerPangkatGolonganUpsert(
    form: {
        id?: string | null;
        id_pangkat_golongan?: number | null;
        nama_pangkat?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiPangkatGolongan }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_pangkat_golongan: Number(form.id_pangkat_golongan),
            nama_pangkat: form.nama_pangkat,
        };
...
export async function FeederReferensiControllerPangkatGolonganDelete(
    props: { id: string },
...
export async function FeederReferensiControllerPangkatGolonganList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_pangkat || String(item.id),
        }));
        return {
            code: 200,
            message: items,
