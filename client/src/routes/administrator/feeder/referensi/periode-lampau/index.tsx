...
import type { FeederReferensiPeriodeLampau } from '~/models/feeder/referensi/PeriodeLampau';
import {
    FeederReferensiControllerPeriodeLampauIndex,
    FeederReferensiControllerPeriodeLampauUpsert,
    FeederReferensiControllerPeriodeLampauDelete,
} from '~/controllers/feeder/referensi/FeederReferensiPeriodeLampauController';

export default function FeederReferensiPeriodeLampauPage() {
    const [items, setItems] = createSignal<FeederReferensiPeriodeLampau[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiPeriodeLampau | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerPeriodeLampauIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiPeriodeLampau[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.program_studi || '').toLowerCase();
                    const bVal = String(b.program_studi || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi periode lampau from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiPeriodeLampau) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiPeriodeLampau) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.program_studi || currentForm.program_studi.trim() === '') {
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerPeriodeLampauUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerPeriodeLampauDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi rekam jejak riwayat periode perkuliahan masa lampau.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            id="input-search-periode-lampau"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-periode-lampau"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Semester</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Program Studi</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.program_studi}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.program_studi}</h3>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                onInput={(e) => setFormData({ ...formData(), semester: e.currentTarget.value })}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().program_studi ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().program_studi}
                                onInput={(e) => setFormData({ ...formData(), program_studi: e.currentTarget.value })}
                            />
                            {formErrors().program_studi && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().program_studi}</p>
                            )}
                        </div>
