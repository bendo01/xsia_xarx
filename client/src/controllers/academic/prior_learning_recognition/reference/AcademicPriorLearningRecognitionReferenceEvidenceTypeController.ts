...
export async function AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicPriorLearningRecognitionReferenceEvidenceType>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/prior-learning-recognition/reference/evidence-types?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicPriorLearningRecognitionReferenceEvidenceType }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/prior-learning-recognition/reference/evidence-types/${form.id}`
            : `${getBaseUrl()}/academic/prior-learning-recognition/reference/evidence-types`;
...
export async function AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/prior-learning-recognition/reference/evidence-types/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
