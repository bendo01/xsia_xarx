...
const path = 'feeder/referensi/jalur-masuk';
...
export async function FeederReferensiControllerJalurMasukIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJalurMasuk>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJalurMasukIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJalurMasukUpsert(
    form: {
        id?: string | null;
        id_jalur_masuk?: number | null;
        nama_jalur_masuk?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJalurMasuk }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jalur_masuk: Number(form.id_jalur_masuk),
            nama_jalur_masuk: form.nama_jalur_masuk,
        };
...
export async function FeederReferensiControllerJalurMasukDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJalurMasukList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jalur_masuk || String(item.id),
        }));
        return {
            code: 200,
            message: items,
