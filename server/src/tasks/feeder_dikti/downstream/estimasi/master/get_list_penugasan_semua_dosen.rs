...
const TASK_NAME: &str = "EstimateListPenugasanSemuaDosen";
const API_ACTION: &str = "GetListPenugasanSemuaDosen";
...
pub struct EstimateListPenugasanSemuaDosen;

impl EstimateListPenugasanSemuaDosen {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputListPenugasanSemuaDosen) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[ModelInputListPenugasanSemuaDosen],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<ModelInputListPenugasanSemuaDosen>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateListPenugasanSemuaDosen {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetListPenugasanSemuaDosen data from Feeder Dikti"
    }
