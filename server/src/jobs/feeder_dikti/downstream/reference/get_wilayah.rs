...
pub struct WorkerArgs {
    pub records: Vec<GetWilayahResponse>,
}
...
pub async fn start_worker(
    ...
    let worker = WorkerBuilder::new("xsia-xarx:feeder_dikti:downstream:reference:get_wilayah")
        .data(db)
        .backend(storage)
        .build_fn(handle_job);
...
impl Worker {
    ...
    pub async fn upsert_record(txn: &DatabaseTransaction, record: &GetWilayahResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
