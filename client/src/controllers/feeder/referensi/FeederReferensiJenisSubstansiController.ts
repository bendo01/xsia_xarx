...
const path = 'feeder/referensi/jenis-substansi';
...
export async function FeederReferensiControllerJenisSubstansiIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenisSubstansi>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenisSubstansiIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenisSubstansiUpsert(
    form: {
        id?: string | null;
        id_jenis_substansi?: string | null;
        nama_jenis_substansi?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenisSubstansi }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenis_substansi: form.id_jenis_substansi,
            nama_jenis_substansi: form.nama_jenis_substansi,
        };
...
export async function FeederReferensiControllerJenisSubstansiDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenisSubstansiList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jenis_substansi || String(item.id),
        }));
        return {
            code: 200,
            message: items,
