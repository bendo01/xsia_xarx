...
use crate::dtos::feeder::master::mahasiswa_bimbingan_dosen::{
    CreateMahasiswaBimbinganDosenRequest, MahasiswaBimbinganDosenQuery, MahasiswaBimbinganDosenResponse, PaginatedMahasiswaBimbinganDosenResponse,
    UpdateMahasiswaBimbinganDosenRequest,
};
use crate::dtos::common::reference::MessageResponse;
use crate::models::feeder::master::mahasiswa_bimbingan_dosen as entity_mod;

#[endpoint(tags("Feeder - Master - MahasiswaBimbinganDosen"), status_codes(200, 500))]
pub async fn list_mahasiswa_bimbingan_dosen(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<PaginatedMahasiswaBimbinganDosenResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        StatusError::internal_server_error().brief("Database connection missing")
    })?;

    let query: MahasiswaBimbinganDosenQuery = req.parse_queries().unwrap_or_default();
    ...
    let data = items.into_iter().map(|item| MahasiswaBimbinganDosenResponse {
            id: item.id,
    ...
    Ok(Json(PaginatedMahasiswaBimbinganDosenResponse {
        data,
        total,
        page,
        page_size,
...
#[endpoint(tags("Feeder - Master - MahasiswaBimbinganDosen"), status_codes(200, 400, 404, 500))]
pub async fn get_mahasiswa_bimbingan_dosen(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<MahasiswaBimbinganDosenResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        StatusError::internal_server_error().brief("Database connection missing")
    })?;
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .ok_or_else(|| StatusError::not_found().brief("MahasiswaBimbinganDosen not found"))?;

    Ok(Json(MahasiswaBimbinganDosenResponse {
            id: item.id,
            id_aktivitas: item.id_aktivitas,
            judul: item.judul,
            id_bimbing_mahasiswa: item.id_bimbing_mahasiswa,
...
}#[endpoint(tags("Feeder - Master - MahasiswaBimbinganDosen"), status_codes(200, 400, 500))]
pub async fn create_mahasiswa_bimbingan_dosen(
        req: &mut Request,
        depot: &mut Depot,
) -> Result<Json<MahasiswaBimbinganDosenResponse>, StatusError> {
        let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        ...
        let payload: CreateMahasiswaBimbinganDosenRequest = req.parse_json().await.map_err(|e| {
...
#[endpoint(tags("Feeder - Master - MahasiswaBimbinganDosen"), status_codes(200, 400, 404, 500))]
pub async fn update_mahasiswa_bimbingan_dosen(
        req: &mut Request,
        depot: &mut Depot,
) -> Result<Json<MahasiswaBimbinganDosenResponse>, StatusError> {
        let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        ...
        let payload: UpdateMahasiswaBimbinganDosenRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
...
#[endpoint(tags("Feeder - Master - MahasiswaBimbinganDosen"), status_codes(200, 400, 404, 500))]
pub async fn delete_mahasiswa_bimbingan_dosen(
        req: &mut Request,
        depot: &mut Depot,
        ...
        Ok(Json(MessageResponse {
            message: "MahasiswaBimbinganDosen deleted successfully".to_string(),
        }))
}
