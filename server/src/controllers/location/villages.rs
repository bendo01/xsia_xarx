...
pub async fn list_villages(
    ...
    let data = items.into_iter().map(|item| VillageResponse {
            id: item.id,
...
pub async fn create_village(
        ...
        let payload: CreateVillageRequest = req.parse_json().await.map_err(|e| {
...
pub async fn update_village(
        ...
        let payload: UpdateVillageRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
