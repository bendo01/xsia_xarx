...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_062236_schema_academic_candidate_master_table_candidate_unit"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_candidate_master"), Alias::new("candidate_unit")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("candidate_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("unit_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("registration_category_id").uuid().not_null(),
                    )
                    .col(
