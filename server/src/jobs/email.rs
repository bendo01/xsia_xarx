...
pub async fn send_email(job: EmailJob) -> Result<(), std::io::Error> {
    ...
    let email = Message::builder()
        .from(format!("{} <{}>", email_config.from_name, email_config.from_email).parse().map_err(|e: lettre::address::AddressError| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?)
        .to(job.to.parse().map_err(|e: lettre::address::AddressError| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?)
        .subject(job.subject)
        .body(job.body)
