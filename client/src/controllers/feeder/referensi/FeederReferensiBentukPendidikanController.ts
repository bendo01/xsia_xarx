...
const path = 'feeder/referensi/bentuk-pendidikan';
...
export async function FeederReferensiControllerBentukPendidikanIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiBentukPendidikan>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerBentukPendidikanIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerBentukPendidikanUpsert(
    form: {
        id?: string | null;
        id_bentuk_pendidikan?: number | null;
        nama_bentuk_pendidikan?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiBentukPendidikan }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_bentuk_pendidikan: Number(form.id_bentuk_pendidikan),
            nama_bentuk_pendidikan: form.nama_bentuk_pendidikan,
        };
...
export async function FeederReferensiControllerBentukPendidikanDelete(
    props: { id: string },
...
export async function FeederReferensiControllerBentukPendidikanList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_bentuk_pendidikan || String(item.id),
        }));
        return {
            code: 200,
            message: items,
