...
impl EstimateListKelasKuliah {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputListKelasKuliah) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[ModelInputListKelasKuliah],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<ModelInputListKelasKuliah>(
            InputRequestData {
...
impl Task for EstimateListKelasKuliah {
    ...
    fn description(&self) -> &str {
        "Fetch and process GetListKelasKuliah data from Feeder Dikti"
    }
