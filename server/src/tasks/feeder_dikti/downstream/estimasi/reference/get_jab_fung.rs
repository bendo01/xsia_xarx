...
use crate::models::feeder::referensi::jabatan_fungsional as jabatan_fungsional;
use crate::tasks::feeder_dikti::downstream::feeder_request::{InputRequestData, RequestData};
use crate::tasks::Task;

// Configuration constants
const TASK_NAME: &str = "EstimateGetJabfung";
const API_ACTION: &str = "GetJabatanFungsional";
...
pub struct GetJabatanFungsionalResponse {
    pub id_jabatan_fungsional: Option<String>,
    pub nama_jabatan_fungsional: String,
}

pub struct EstimateGetJabfung;

impl EstimateGetJabfung {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJabatanFungsionalResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let existing = jabatan_fungsional::Entity::find()
            .filter(jabatan_fungsional::Column::DeletedAt.is_null())
            .filter(
                jabatan_fungsional::Column::IdJabatanFungsional.eq(id_jabatan_fungsional.clone()),
            )
            .one(txn)
            .await?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJabatanFungsionalResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJabatanFungsionalResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJabfung {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJabatanFungsional data from Feeder Dikti"
    }
