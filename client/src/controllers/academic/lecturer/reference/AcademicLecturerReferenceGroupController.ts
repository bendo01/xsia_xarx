...
export async function AcademicLecturerReferenceControllerGroupIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicLecturerReferenceGroup>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/lecturer/reference/groups?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicLecturerReferenceControllerGroupIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicLecturerReferenceControllerGroupUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicLecturerReferenceGroup }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/lecturer/reference/groups/${form.id}`
            : `${getBaseUrl()}/academic/lecturer/reference/groups`;
...
export async function AcademicLecturerReferenceControllerGroupDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/lecturer/reference/groups/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
