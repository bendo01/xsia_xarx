...
export async function AcademicStudentReferenceControllerResignStatusIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicStudentReferenceResignStatus>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/student/reference/resign-statuses?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicStudentReferenceControllerResignStatusIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicStudentReferenceControllerResignStatusUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicStudentReferenceResignStatus }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/student/reference/resign-statuses/${form.id}`
            : `${getBaseUrl()}/academic/student/reference/resign-statuses`;
...
export async function AcademicStudentReferenceControllerResignStatusDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/student/reference/resign-statuses/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
