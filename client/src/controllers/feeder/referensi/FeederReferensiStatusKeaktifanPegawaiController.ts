...
const path = 'feeder/referensi/status-keaktifan-pegawai';
...
export async function FeederReferensiControllerStatusKeaktifanPegawaiIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiStatusKeaktifanPegawai>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerStatusKeaktifanPegawaiIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerStatusKeaktifanPegawaiUpsert(
    form: {
        id?: string | null;
        id_status_aktif?: string | null;
        nama_status_aktif?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiStatusKeaktifanPegawai }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_status_aktif: form.id_status_aktif,
            nama_status_aktif: form.nama_status_aktif,
        };
...
export async function FeederReferensiControllerStatusKeaktifanPegawaiDelete(
    props: { id: string },
...
export async function FeederReferensiControllerStatusKeaktifanPegawaiList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_status_aktif || String(item.id),
        }));
        return {
            code: 200,
            message: items,
