...
import type { AcademicPriorLearningRecognitionReferenceEvidenceType } from '~/models/academic/prior_learning_recognition/reference/EvidenceType';
import type { TypeInputEntityReferenceForm } from '~/lib/types';
import {
    AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeIndex,
    AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeUpsert,
    AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeDelete,
} from '~/controllers/academic/prior_learning_recognition/reference/AcademicPriorLearningRecognitionReferenceEvidenceTypeController';

export default function AcademicPriorLearningRecognitionReferenceEvidenceTypePage() {
    const [items, setItems] = createSignal<AcademicPriorLearningRecognitionReferenceEvidenceType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<AcademicPriorLearningRecognitionReferenceEvidenceType | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as AcademicPriorLearningRecognitionReferenceEvidenceType[];
            ...
            console.error('Error loading evidence type data:', error);
            toast.danger('Failed to load evidence type data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: AcademicPriorLearningRecognitionReferenceEvidenceType) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: AcademicPriorLearningRecognitionReferenceEvidenceType) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Evidence type name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Evidence type reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create evidence type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Evidence type reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update evidence type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await AcademicPriorLearningRecognitionReferenceControllerEvidenceTypeDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Evidence type reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete evidence type reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        RPL Evidence Type / Jenis Bukti RPL
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference RPL evidence document types (e.g. Sertifikat Profesi, SK Pengangkatan, Portofolio Karya).
                    </p>
                </div>
                <div class="mt-4 sm:mt-0 flex items-center gap-2 justify-end">
                    <button
                        ...
                        id="btn-add-rpl-evidence-type"
                    >
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M5 12h14" />
                            <path d="M12 5v14" />
                        </svg>
                        <span>Add Evidence Type</span>
                    </button>
                </div>
            </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by evidence type (e.g. Sertifikat Profesi, Portofolio)..."
                            onInput={handleSearch}
                            id="input-search-rpl-evidence-type"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-rpl-evidence-type"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-rpl-evidence-type"
                        >
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Evidence Type Name</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No evidence type records found</p>
                                                        <p class="text-xs">Click "Add Evidence Type" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                        ...
                        <Show
                            ...
                            fallback={
                                <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                    No evidence type records found.
                                </div>
                            }
                        >
                            <For each={items()}>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add RPL Evidence Type Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new RPL evidence type reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Evidence Type Name <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-xs bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                ...
                                placeholder="e.g. Sertifikat Uji Kompetensi"
                                value={formData().name}
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit RPL Evidence Type Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify RPL evidence type reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="flex items-center gap-3 pb-3 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">
                                This evidence type record will be removed.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="py-4">
                        ...
                        <table class="w-full text-xs text-left border border-neutral-200 dark:border-neutral-700">
                            <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                ...
                                <tr class="bg-neutral-50 dark:bg-neutral-800/50">
                                    <td class="px-3 py-2 font-semibold text-neutral-600 dark:text-neutral-300">Evidence Type Name</td>
