...
import {
    LocationSubDistrictControllerIndex,
    LocationSubDistrictControllerUpsert,
    LocationSubDistrictControllerDelete,
} from '~/controllers/location/LocationSubDistrictController';
...
export default function LocationSubDistrictPage() {
    const [items, setItems] = createSignal<LocationSubDistrict[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<LocationSubDistrict | null>(null);
    ...
    const loadOptions = async () => {
        try {
            const [regRes, provRes, countryRes] = await Promise.all([
            ...
            if (regRes && regRes.data) {
                ...
                for (const reg of regRes.data) {
                    map.set(reg.id, reg);
                    opts.push({ id: reg.id, value: reg.id, label: reg.name || reg.code || reg.id });
                }
                setRegenciesMap(map);
                setRegencyOptions(opts);
            }
    ...
    const getRegency = (regencyId?: string | null): LocationRegency | undefined => {
        if (!regencyId) return undefined;
        const reg = regenciesMap().get(regencyId);
        if (!reg) {
            fetchRegencyById(regencyId);
            return undefined;
        }
        return reg;
    };

    const getProvince = (regencyId?: string | null): LocationProvince | undefined => {
        const reg = getRegency(regencyId);
    ...
    const getCountry = (regencyId?: string | null): LocationCountry | undefined => {
        const prov = getProvince(regencyId);
    ...
    const getRegencyName = (regencyId?: string | null): string => {
        const reg = getRegency(regencyId);
        return reg?.name || '-';
    };

    const getProvinceName = (regencyId?: string | null): string => {
        const prov = getProvince(regencyId);
        return prov?.name || '-';
    };

    const getCountryName = (regencyId?: string | null): string => {
        const country = getCountry(regencyId);
        return country?.name || '-';
    };
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await LocationSubDistrictControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as LocationSubDistrict[];
                ...
                // Pre-trigger hierarchy fetching for any missing regencies in the current page
                for (const item of data) {
                    if (item.regency_id && !regenciesMap().has(item.regency_id)) {
                        fetchRegencyById(item.regency_id);
                    }
                }
            } else {
                setItems([]);
            ...
            console.error('Error loading sub-district data:', error);
            toast.danger('Failed to load sub-district data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openCreateModal = () => {
        setFormData({
            ...
            code: '',
            name: '',
            regency_id: regencyOptions()[0]?.id || '',
            dikti_code: '',
            slug: '',
            alt_slug: '',
        });
    ...
    const openEditModal = (item: LocationSubDistrict) => {
        setFormData({
            id: item.id,
            code: item.code || '',
            name: item.name || '',
    ...
    const openDeleteModal = (item: LocationSubDistrict) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.code || currentForm.code.trim() === '') {
            errors.code = 'Sub-district code is required.';
        }

        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Sub-district name is required.';
        }

        if (!currentForm.regency_id || currentForm.regency_id.trim() === '') {
            errors.regency_id = 'Regency / Kabupaten is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationSubDistrictControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Sub-district created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create sub-district.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await LocationSubDistrictControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Sub-district updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update sub-district.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await LocationSubDistrictControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Sub-district deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete sub-district.', 5000);
            }
        } catch (error: any) {
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new sub-district / kecamatan location record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Sub-District Code <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Regency / Kabupaten / Kota <span class="text-red-500">*</span>
                            </label>
                            <select
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().regency_id ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                value={formData().regency_id}
                                onChange={(e) => setFormData({ ...formData(), regency_id: e.currentTarget.value })}
                            >
                            ...
                            {formErrors().regency_id && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().regency_id}</p>
                            )}

                            {/* Location Hierarchy Preview */}
                            <Show when={formData().regency_id}>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            >
                                {isSubmitting() ? 'Creating...' : 'Save Sub-District'}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify sub-district properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleEditSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            <button
                                ...
                                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-none transition-colors cursor-pointer"
                            >
                                {isSubmitting() ? 'Updating...' : 'Update Sub-District'}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Are you sure you want to delete this sub-district?
                            </p>
                        </div>
                    </div>
