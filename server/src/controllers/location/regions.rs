...
use crate::models::location::regions as entity_mod;
...
pub async fn list_regions(
    ...
    let data = items.into_iter().map(|item| RegionResponse {
            id: item.id,
            code: item.code,
...
pub async fn create_region(
        ...
        let payload: CreateRegionRequest = req.parse_json().await.map_err(|e| {
...
pub async fn update_region(
        ...
        let payload: UpdateRegionRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
