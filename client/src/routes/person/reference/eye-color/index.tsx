...
import type { PersonReferenceEyeColor } from '~/models/person/reference/EyeColor';
...
export default function PersonReferenceEyeColorPage() {
    const [items, setItems] = createSignal<PersonReferenceEyeColor[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<PersonReferenceEyeColor | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await PersonReferenceControllerEyeColorIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as PersonReferenceEyeColor[];
            ...
            console.error('Error loading eye color data:', error);
            toast.danger('Failed to load eye color data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: PersonReferenceEyeColor) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: PersonReferenceEyeColor) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Eye color name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerEyeColorUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Eye color reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create eye color reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerEyeColorUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Eye color reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update eye color reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await PersonReferenceControllerEyeColorDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Eye color reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete eye color reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for eye colors.
                    </p>
                </div>
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Eye Color Name</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No eye color records found</p>
                                                        <p class="text-xs">Click "Add Eye Color" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new eye color reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Eye Color Name <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify eye color reference attributes.
                            </p>
                        </div>
                        <button
                            type="button"
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">Delete Eye Color</h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">This action cannot be undone.</p>
                        </div>
                    </div>

                    <p class="text-sm text-neutral-600 dark:text-neutral-400 mb-6">
                        Are you sure you want to delete the eye color <strong class="text-neutral-900 dark:text-white">"{selectedItem()?.name}"</strong>?
                    </p>
