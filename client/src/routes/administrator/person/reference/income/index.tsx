...
import type { PersonReferenceIncome } from '~/models/person/reference/Income';
...
export default function PersonReferenceIncomePage() {
    const [items, setItems] = createSignal<PersonReferenceIncome[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<PersonReferenceIncome | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await PersonReferenceControllerIncomeIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as PersonReferenceIncome[];
            ...
            toast.danger('Failed to load income data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: PersonReferenceIncome) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: PersonReferenceIncome) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Income range name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerIncomeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Income reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create income reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerIncomeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Income reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update income reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await PersonReferenceControllerIncomeDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Income reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete income reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for personal and family income ranges.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by income range (e.g. < 1 Juta, 1 - 3 Juta)..."
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Income Bracket</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No income ranges found</p>
                                                        <p class="text-xs">Click "Add Income Range" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new income bracket reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Income Range Name <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-xs bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                ...
                                placeholder="e.g. < Rp 1.000.000, Rp 1.000.000 - Rp 3.000.000..."
                                value={formData().name}
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify income range reference details.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="mt-3 border border-neutral-200 dark:border-neutral-700 overflow-hidden">
                        <table class="w-full text-sm text-left">
                            <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                ...
                                <tr class="bg-neutral-50/50 dark:bg-neutral-800/30">
                                    <td class="px-3 py-2 font-medium text-neutral-500 dark:text-neutral-400">Income Name</td>
