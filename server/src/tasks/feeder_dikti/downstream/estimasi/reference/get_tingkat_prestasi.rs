...
use crate::models::feeder::referensi::tingkat_prestasi as tingkat_prestasi;
use crate::tasks::feeder_dikti::downstream::feeder_request::{InputRequestData, RequestData};
use crate::tasks::Task;

// Configuration constants
const TASK_NAME: &str = "EstimateGetTingkatPrestasi";
const API_ACTION: &str = "GetTingkatPrestasi";
...
pub struct GetTingkatPrestasiResponse {
    pub id_tingkat_prestasi: Option<i32>,
    pub nama_tingkat_prestasi: Option<String>,
}

pub struct EstimateGetTingkatPrestasi;

impl EstimateGetTingkatPrestasi {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetTingkatPrestasiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_tingkat_prestasi = record
            .id_tingkat_prestasi
            .clone()
            .ok_or_else(|| DbErr::Custom("id_tingkat_prestasi is missing".to_string()))?;

        let sync_time = Local::now().naive_local();

        let existing = tingkat_prestasi::Entity::find()
            .filter(tingkat_prestasi::Column::DeletedAt.is_null())
            .filter(tingkat_prestasi::Column::IdTingkatPrestasi.eq(id_tingkat_prestasi))
            .one(txn)
            .await?;

        let action = if let Some(existing_record) = existing {
            let mut active: tingkat_prestasi::ActiveModel = existing_record.into_active_model();

            // Update fields
            active.nama_tingkat_prestasi = Set(record.nama_tingkat_prestasi.clone());
            ...
            let new_record = tingkat_prestasi::ActiveModel {
                id: Set(pk_id),
                id_tingkat_prestasi: Set(Some(id_tingkat_prestasi)),
                nama_tingkat_prestasi: Set(record.nama_tingkat_prestasi.clone()),
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetTingkatPrestasiResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetTingkatPrestasiResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetTingkatPrestasi {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetTingkatPrestasi data from Feeder Dikti"
    }
