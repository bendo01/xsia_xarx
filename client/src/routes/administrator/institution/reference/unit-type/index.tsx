...
import type { InstitutionReferenceUnitType } from '~/models/institution/reference/UnitType';
import type { TypeInputEntityReferenceForm } from '~/lib/types';
import {
    InstitutionReferenceControllerUnitTypeIndex,
    InstitutionReferenceControllerUnitTypeUpsert,
    InstitutionReferenceControllerUnitTypeDelete,
} from '~/controllers/institution/reference/InstitutionReferenceUnitTypeController';

export default function InstitutionReferenceUnitTypePage() {
    const [items, setItems] = createSignal<InstitutionReferenceUnitType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<InstitutionReferenceUnitType | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await InstitutionReferenceControllerUnitTypeIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as InstitutionReferenceUnitType[];
            ...
            console.error('Error loading unit type data:', error);
            toast.danger('Failed to load unit type data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: InstitutionReferenceUnitType) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: InstitutionReferenceUnitType) => {
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await InstitutionReferenceControllerUnitTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Unit type reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create unit type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await InstitutionReferenceControllerUnitTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Unit type reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update unit type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await InstitutionReferenceControllerUnitTypeDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Unit type reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete unit type reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for organizational units, departments, faculties, and divisions.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by unit type name (e.g. Fakultas, Program Studi, Biro, Lembaga)..."
                            onInput={handleSearch}
                            id="input-search-unit-type"
                        />
                    </div>
                </div>
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Unit Type Name</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No unit type records found</p>
                                                        <p class="text-xs">Click "Add Unit Type" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Unit Type Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new unit type reference record.
                            </p>
                        </div>
                        <button
                            type="button"
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Unit Type Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify unit type reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="flex items-center gap-3 pb-3 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">
                                This unit type record will be removed.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="py-4">
                        ...
                        <table class="w-full text-xs text-left border border-neutral-200 dark:border-neutral-700">
                            <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                ...
                                <tr class="bg-neutral-50 dark:bg-neutral-800/50">
                                    <td class="px-3 py-2 font-semibold text-neutral-600 dark:text-neutral-300">Unit Type Name</td>
