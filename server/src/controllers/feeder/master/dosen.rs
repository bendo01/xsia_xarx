...
use crate::dtos::feeder::master::dosen::{
    CreateDosenRequest, DosenQuery, DosenResponse, PaginatedDosenResponse,
    UpdateDosenRequest,
};
use crate::dtos::common::reference::MessageResponse;
use crate::models::feeder::master::dosen as entity_mod;

#[endpoint(tags("Feeder - Master - Dosen"), status_codes(200, 500))]
pub async fn list_dosen(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<PaginatedDosenResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        StatusError::internal_server_error().brief("Database connection missing")
    })?;

    let query: DosenQuery = req.parse_queries().unwrap_or_default();
    ...
    let data = items.into_iter().map(|item| DosenResponse {
            id: item.id,
    ...
    Ok(Json(PaginatedDosenResponse {
        data,
        total,
        page,
        page_size,
