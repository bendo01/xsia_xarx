...
export async function AcademicCampaignReferenceControllerSubstanceIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCampaignReferenceSubstance>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/substances?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCampaignReferenceControllerSubstanceIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCampaignReferenceControllerSubstanceUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCampaignReferenceSubstance }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/campaign/reference/substances/${form.id}`
            : `${getBaseUrl()}/academic/campaign/reference/substances`;
...
export async function AcademicCampaignReferenceControllerSubstanceDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/substances/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
