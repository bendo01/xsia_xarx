...
export async function AcademicCourseReferenceControllerCompetenceIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCourseReferenceCompetence>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/competences?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCourseReferenceControllerCompetenceIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCourseReferenceControllerCompetenceUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCourseReferenceCompetence }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/course/reference/competences/${form.id}`
            : `${getBaseUrl()}/academic/course/reference/competences`;
...
export async function AcademicCourseReferenceControllerCompetenceDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/competences/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
