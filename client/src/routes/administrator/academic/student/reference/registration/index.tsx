...
import {
    AcademicStudentReferenceControllerRegistrationIndex,
    AcademicStudentReferenceControllerRegistrationUpsert,
    AcademicStudentReferenceControllerRegistrationDelete,
} from '~/controllers/academic/student/reference/AcademicStudentReferenceRegistrationController';

export default function AcademicStudentReferenceRegistrationPage() {
    const [items, setItems] = createSignal<AcademicStudentReferenceRegistration[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<AcademicStudentReferenceRegistration | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await AcademicStudentReferenceControllerRegistrationIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as AcademicStudentReferenceRegistration[];
            ...
            console.error('Error loading student registration status data:', error);
            toast.danger('Failed to load student registration status data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: AcademicStudentReferenceRegistration) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: AcademicStudentReferenceRegistration) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Registration status name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicStudentReferenceControllerRegistrationUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Student registration status created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create student registration status.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicStudentReferenceControllerRegistrationUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Student registration status updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update student registration status.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await AcademicStudentReferenceControllerRegistrationDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Student registration status deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete student registration status.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Student Registration Status / Status Registrasi Mahasiswa
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference student periodic registration statuses (e.g. Terdaftar, Belum Registrasi, Ditolak, Cuti Terencana).
                    </p>
                </div>
                <div class="mt-4 sm:mt-0 flex items-center gap-2 justify-end">
                    <button
                        ...
                        id="btn-add-registration"
                    >
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M5 12h14" />
                            <path d="M12 5v14" />
                        </svg>
                        <span>Add Registration Status</span>
                    </button>
                </div>
            </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by registration status (e.g. Terdaftar)..."
                            onInput={handleSearch}
                            id="input-search-registration"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-registration"
                        >
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Registration Status Name</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No student registration status records found</p>
                                                        <p class="text-xs">Click "Add Registration Status" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                        ...
                        <Show
                            ...
                            fallback={
                                <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                    No student registration status records found.
                                </div>
                            }
                        >
                            <For each={items()}>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Student Registration Status Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new student registration status reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Registration Status Name <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-xs bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                ...
                                placeholder="e.g. Terdaftar (Registered)"
                                value={formData().name}
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Student Registration Status Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify student registration status reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="flex items-center gap-3 pb-3 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">
                                This student registration status record will be removed.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="py-4">
                        ...
                        <table class="w-full text-xs text-left border border-neutral-200 dark:border-neutral-700">
                            <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                                ...
                                <tr class="bg-neutral-50 dark:bg-neutral-800/50">
                                    <td class="px-3 py-2 font-semibold text-neutral-600 dark:text-neutral-300">Registration Status</td>
