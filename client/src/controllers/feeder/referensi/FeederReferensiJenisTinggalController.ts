...
const path = 'feeder/referensi/jenis-tinggal';
...
export async function FeederReferensiControllerJenisTinggalIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenisTinggal>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenisTinggalIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenisTinggalUpsert(
    form: {
        id?: string | null;
        id_jenis_tinggal?: number | null;
        nama_jenis_tinggal?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenisTinggal }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenis_tinggal: Number(form.id_jenis_tinggal),
            nama_jenis_tinggal: form.nama_jenis_tinggal,
        };
...
export async function FeederReferensiControllerJenisTinggalDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenisTinggalList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jenis_tinggal || String(item.id),
        }));
        return {
            code: 200,
            message: items,
