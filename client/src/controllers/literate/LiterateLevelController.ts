...
export async function LiterateLevelControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<LiterateLevel>> {
    try {
        ...
        console.error('Error in LiterateLevelControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function LiterateLevelControllerUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: LiterateLevel }> {
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update level.' : 'Failed to create level.'),
            };
        }

        return {
            is_error: false,
            message: isUpdate ? 'Level updated successfully.' : 'Level created successfully.',
            data: resJson,
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while saving level.',
        };
    }
}

export async function LiterateLevelControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete level.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Level deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting level.',
        };
    }
}

export async function LiterateLevelControllerList(): Promise<{
    code: number;
