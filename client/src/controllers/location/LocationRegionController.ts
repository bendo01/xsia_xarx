...
export async function LocationRegionControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<LocationRegion>> {
    try {
        ...
        console.error('Error in LocationRegionControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function LocationRegionControllerUpsert(
    form: { id?: string | null; code?: number | string | null; alphabet_code?: string | null; name: string; slug?: string | null; alt_slug?: string | null },
): Promise<{ is_error: boolean; message: string; data?: LocationRegion }> {
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update region.' : 'Failed to create region.'),
            };
        }

        return {
            is_error: false,
            message: isUpdate ? 'Region updated successfully.' : 'Region created successfully.',
            data: resJson,
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while saving region.',
        };
    }
}

export async function LocationRegionControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete region.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Region deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting region.',
        };
    }
}

export async function LocationRegionControllerList(): Promise<{
    code: number;
