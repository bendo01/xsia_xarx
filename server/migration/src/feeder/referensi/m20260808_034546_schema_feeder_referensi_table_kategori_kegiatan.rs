...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260808_034546_schema_feeder_referensi_table_kategori_kegiatan"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("feeder_referensi"), Alias::new("kategori_kegiatan")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("id_kategori_kegiatan").integer(),
                    )
                    .col(
                        ColumnDef::new("nama_kategori_kegiatan").text(),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("feeder_referensi_kategori_kegiatan_pkey")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
