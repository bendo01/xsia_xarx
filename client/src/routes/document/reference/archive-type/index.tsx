...
import {
    DocumentArchiveTypeControllerIndex,
    DocumentArchiveTypeControllerUpsert,
    DocumentArchiveTypeControllerDelete,
} from '~/controllers/document/DocumentArchiveTypeController';

export default function DocumentReferenceArchiveTypePage() {
    const [items, setItems] = createSignal<DocumentArchiveType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<DocumentArchiveType | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await DocumentArchiveTypeControllerIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as DocumentArchiveType[];
            ...
            console.error('Error loading archive type data:', error);
            toast.danger('Failed to load archive type data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: DocumentArchiveType) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: DocumentArchiveType) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (currentForm.code !== '' && isNaN(Number(currentForm.code))) {
            errors.code = 'Code must be numeric if provided.';
        }
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Archive type name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await DocumentArchiveTypeControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Archive type created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create archive type.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await DocumentArchiveTypeControllerUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Archive type updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update archive type.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await DocumentArchiveTypeControllerDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Archive type deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete archive type.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Archive Type / Jenis Arsip Dokumen
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage document classification categories, archival formats, and file categorization.
                    </p>
                </div>
                <div class="mt-4 sm:mt-0 flex items-center gap-2 justify-end">
                    <button
                        ...
                        id="btn-add-archive-type"
                    >
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M5 12h14" />
                            <path d="M12 5v14" />
                        </svg>
                        <span>Add Archive Type</span>
                    </button>
                </div>
            </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by archive type name (e.g. Ijazah, Transkrip, Sertifikat)..."
                            onInput={handleSearch}
                            id="input-search-archive-type"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-archive-type"
                        >
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No archive type records found</p>
                                                        <p class="text-xs">Click "Add Archive Type" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Register a new document archival classification.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            >
                                {isSubmitting() ? 'Creating...' : 'Save Archive Type'}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify document classification properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleEditSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex items-center justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            <button
                                ...
                                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-none transition-colors cursor-pointer"
                            >
                                {isSubmitting() ? 'Updating...' : 'Update Archive Type'}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Are you sure you want to delete this archive type?
                            </p>
                        </div>
                    </div>
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.name} ({selectedItem()?.alphabet_code || selectedItem()?.code || ''})
                    </p>
