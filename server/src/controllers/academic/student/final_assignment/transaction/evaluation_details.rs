...
use crate::dtos::academic::student::final_assignment::transaction::evaluation_details::{
...
use crate::models::academic::student::final_assignment::transaction::evaluation_details as entity_mod;
...
pub async fn list_evaluation_details(
    ...
    let data = items.into_iter().map(|item| EvaluationDetailResponse {
            id: item.id,
            evaluation_summary_id: item.evaluation_summary_id,
...
pub async fn create_evaluation_detail(
    ...
    let active_model = entity_mod::ActiveModel {
        id: Set(new_id),
        evaluation_summary_id: Set(payload.evaluation_summary_id),
        adviser_id: Set(payload.adviser_id),
...
pub async fn update_evaluation_detail(
    ...
    if let Some(evaluation_summary_id) = payload.evaluation_summary_id {
        active_model.evaluation_summary_id = Set(evaluation_summary_id);
    }
    if let Some(adviser_id) = payload.adviser_id {
        active_model.adviser_id = Set(adviser_id);
    }
