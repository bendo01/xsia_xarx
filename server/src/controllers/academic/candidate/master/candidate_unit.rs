...
use crate::dtos::academic::candidate::master::candidate_unit::{
    CreateCandidateUnitRequest, CandidateUnitQuery, CandidateUnitResponse, PaginatedCandidateUnitResponse,
    UpdateCandidateUnitRequest,
};
use crate::dtos::common::reference::MessageResponse;
use crate::models::academic::candidate::master::candidate_unit as entity_mod;
...
pub async fn list_candidate_unit(
    ...
    let query: CandidateUnitQuery = req.parse_queries().unwrap_or_default();
    ...
    let data = items.into_iter().map(|item| CandidateUnitResponse {
            id: item.id,
    ...
    Ok(Json(PaginatedCandidateUnitResponse {
        data,
        total,
        page,
        page_size,
...
pub async fn get_candidate_unit(
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .ok_or_else(|| StatusError::not_found().brief("CandidateUnit not found"))?;

    Ok(Json(CandidateUnitResponse {
            id: item.id,
            candidate_id: item.candidate_id,
            unit_id: item.unit_id,
            registration_category_id: item.registration_category_id,
...
pub async fn create_candidate_unit(
        ...
        let payload: CreateCandidateUnitRequest = req.parse_json().await.map_err(|e| {
...
pub async fn update_candidate_unit(
        ...
        let payload: UpdateCandidateUnitRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
    ...
    if let Some(registration_category_id) = payload.registration_category_id {
            active_model.registration_category_id = Set(registration_category_id);
        }
...
pub async fn delete_candidate_unit(
        ...
        Ok(Json(MessageResponse {
            message: "CandidateUnit deleted successfully".to_string(),
        }))
}
