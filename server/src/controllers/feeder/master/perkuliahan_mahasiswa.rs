...
use crate::dtos::feeder::master::perkuliahan_mahasiswa::{
    CreatePerkuliahanMahasiswaRequest, PerkuliahanMahasiswaQuery, PerkuliahanMahasiswaResponse, PaginatedPerkuliahanMahasiswaResponse,
    UpdatePerkuliahanMahasiswaRequest,
};
use crate::dtos::common::reference::MessageResponse;
use crate::models::feeder::master::perkuliahan_mahasiswa as entity_mod;
...
pub async fn list_perkuliahan_mahasiswa(
    ...
    let query: PerkuliahanMahasiswaQuery = req.parse_queries().unwrap_or_default();
    ...
    let data = items.into_iter().map(|item| PerkuliahanMahasiswaResponse {
            id: item.id,
    ...
    Ok(Json(PaginatedPerkuliahanMahasiswaResponse {
        data,
        total,
        page,
        page_size,
...
pub async fn get_perkuliahan_mahasiswa(
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .ok_or_else(|| StatusError::not_found().brief("PerkuliahanMahasiswa not found"))?;

    Ok(Json(PerkuliahanMahasiswaResponse {
            id: item.id,
            created_at: item.created_at,
            updated_at: item.updated_at,
            deleted_at: item.deleted_at,
...
pub async fn create_perkuliahan_mahasiswa(
        ...
        let payload: CreatePerkuliahanMahasiswaRequest = req.parse_json().await.map_err(|e| {
...
pub async fn update_perkuliahan_mahasiswa(
        ...
        let payload: UpdatePerkuliahanMahasiswaRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
...
pub async fn delete_perkuliahan_mahasiswa(
        ...
        Ok(Json(MessageResponse {
            message: "PerkuliahanMahasiswa deleted successfully".to_string(),
        }))
}
