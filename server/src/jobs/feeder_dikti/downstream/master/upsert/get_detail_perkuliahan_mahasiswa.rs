...
pub async fn start_worker(
    ...
    let worker = WorkerBuilder::new("xsia-xarx:feeder_dikti:downstream:master:upsert:get_detail_perkuliahan_mahasiswa")
        .data(db)
        .backend(storage)
        .build_fn(handle_job);
...
impl Worker {
    ...
    pub async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputDetailPerkuliahanMahasiswa) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
