...
import type { FeederReferensiPenghasilan } from '~/models/feeder/referensi/Penghasilan';
import {
    FeederReferensiControllerPenghasilanIndex,
    FeederReferensiControllerPenghasilanUpsert,
    FeederReferensiControllerPenghasilanDelete,
} from '~/controllers/feeder/referensi/FeederReferensiPenghasilanController';

export default function FeederReferensiPenghasilanPage() {
    const [items, setItems] = createSignal<FeederReferensiPenghasilan[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiPenghasilan | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerPenghasilanIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiPenghasilan[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_penghasilan || '').toLowerCase();
                    const bVal = String(b.nama_penghasilan || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi rentang penghasilan from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiPenghasilan) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiPenghasilan) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_penghasilan || currentForm.nama_penghasilan.trim() === '') {
            errors.nama_penghasilan = 'Nama Rentang Penghasilan is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerPenghasilanUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerPenghasilanDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Referensi Rentang Penghasilan
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi rentang skala pendapatan ekonomi orang tua/wali PDDIKTI Feeder.
                    </p>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-penghasilan"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Penghasilan</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Rentang Penghasilan</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_penghasilan}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_penghasilan}</h3>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Referensi Rentang Penghasilan
                            </h2>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                onInput={(e) => setFormData({ ...formData(), id_penghasilan: e.currentTarget.value })}
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Nama Rentang Penghasilan <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_penghasilan ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_penghasilan}
                                onInput={(e) => setFormData({ ...formData(), nama_penghasilan: e.currentTarget.value })}
                            />
                            {formErrors().nama_penghasilan && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_penghasilan}</p>
                            )}
                        </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Referensi Rentang Penghasilan
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
