...
export async function LiterateVarietyControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<LiterateVariety>> {
    try {
        ...
        console.error('Error in LiterateVarietyControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function LiterateVarietyControllerUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: LiterateVariety }> {
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update variety.' : 'Failed to create variety.'),
            };
        }

        return {
            is_error: false,
            message: isUpdate ? 'Variety updated successfully.' : 'Variety created successfully.',
            data: resJson,
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while saving variety.',
        };
    }
}

export async function LiterateVarietyControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete variety.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Variety deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting variety.',
        };
    }
}

export async function LiterateVarietyControllerList(): Promise<{
    code: number;
