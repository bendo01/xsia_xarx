...
import {
    AcademicCandidateReferenceControllerRegistrationTypeIndex,
    AcademicCandidateReferenceControllerRegistrationTypeUpsert,
    AcademicCandidateReferenceControllerRegistrationTypeDelete,
} from '~/controllers/academic/candidate/reference/AcademicCandidateReferenceRegistrationTypeController';

export default function AcademicCandidateReferenceRegistrationTypePage() {
    const [items, setItems] = createSignal<AcademicCandidateReferenceRegistrationType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<AcademicCandidateReferenceRegistrationType | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await AcademicCandidateReferenceControllerRegistrationTypeIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as AcademicCandidateReferenceRegistrationType[];
            ...
            console.error('Error loading registration type data:', error);
            toast.danger('Failed to load registration type data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: AcademicCandidateReferenceRegistrationType) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: AcademicCandidateReferenceRegistrationType) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Registration type name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicCandidateReferenceControllerRegistrationTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Registration type reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create registration type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await AcademicCandidateReferenceControllerRegistrationTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Registration type reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update registration type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await AcademicCandidateReferenceControllerRegistrationTypeDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Registration type reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete registration type reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference types for candidate registration (e.g. Mahasiswa Baru, Pindahan, Alih Jenjang).
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by registration type name (e.g. Mahasiswa Baru, Pindahan)..."
                            onInput={handleSearch}
                            id="input-search-registration-type"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-registration-type"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-registration-type"
                        >
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No registration type records found</p>
                                                        <p class="text-xs">Click "Add Type" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                        ...
                        <Show
                            ...
                            fallback={
                                <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                    No registration type records found.
                                </div>
                            }
                        >
                            <For each={items()}>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Registration Type Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new registration type reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. Mahasiswa Baru"
                                value={formData().name}
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Registration Type Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify registration type reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="flex items-center gap-3 pb-3 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">
                                This registration type record will be removed.
                            </p>
                        </div>
                    </div>
