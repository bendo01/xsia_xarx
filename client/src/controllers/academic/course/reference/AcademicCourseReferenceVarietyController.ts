...
export async function AcademicCourseReferenceControllerVarietyIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCourseReferenceVariety>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/varieties?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCourseReferenceControllerVarietyIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCourseReferenceControllerVarietyUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCourseReferenceVariety }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/course/reference/varieties/${form.id}`
            : `${getBaseUrl()}/academic/course/reference/varieties`;
...
export async function AcademicCourseReferenceControllerVarietyDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/varieties/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
