...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260808_034545_schema_feeder_akumulasi_table_estimasi"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS feeder_akumulasi")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("feeder_akumulasi"), Alias::new("estimasi")))
                    .if_not_exists()
                    .col(
                    ...
                    .col(
                        ColumnDef::new("total_data_per_request").integer().default(0),
                    )
                    .col(
                        ColumnDef::new("last_offset").integer().default(0),
                    )
                    .col(
                        ColumnDef::new("total_data").integer().default(0),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("feeder_akumulasi_estimasi_pkey")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
