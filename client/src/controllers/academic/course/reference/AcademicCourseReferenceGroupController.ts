...
export async function AcademicCourseReferenceControllerGroupIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCourseReferenceGroup>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/groups?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCourseReferenceControllerGroupIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCourseReferenceControllerGroupUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCourseReferenceGroup }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/course/reference/groups/${form.id}`
            : `${getBaseUrl()}/academic/course/reference/groups`;
...
export async function AcademicCourseReferenceControllerGroupDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/groups/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
