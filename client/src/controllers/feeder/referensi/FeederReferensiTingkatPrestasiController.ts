...
const path = 'feeder/referensi/tingkat-prestasi';
...
export async function FeederReferensiControllerTingkatPrestasiIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiTingkatPrestasi>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerTingkatPrestasiIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerTingkatPrestasiUpsert(
    form: {
        id?: string | null;
        id_tingkat_prestasi?: number | null;
        nama_tingkat_prestasi?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiTingkatPrestasi }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_tingkat_prestasi: Number(form.id_tingkat_prestasi),
            nama_tingkat_prestasi: form.nama_tingkat_prestasi,
        };
...
export async function FeederReferensiControllerTingkatPrestasiDelete(
    props: { id: string },
...
export async function FeederReferensiControllerTingkatPrestasiList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_tingkat_prestasi || String(item.id),
        }));
        return {
            code: 200,
            message: items,
