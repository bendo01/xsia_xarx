...
const TASK_NAME: &str = "EstimateDetailPeriodePerkuliahan";
...
impl EstimateDetailPeriodePerkuliahan {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputDetailPeriodePerkuliahan) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        // Start transaction
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[ModelInputDetailPeriodePerkuliahan],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<ModelInputDetailPeriodePerkuliahan>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateDetailPeriodePerkuliahan {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetDetailPeriodePerkuliahan data from Feeder Dikti"
    }
