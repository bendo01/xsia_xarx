...
pub async fn start_worker(
    ...
    let worker = WorkerBuilder::new("xsia-xarx:feeder_dikti:downstream:master:upsert:get_list_periode_perkuliahan")
        .data(db)
        .backend(storage)
        .build_fn(handle_job);
...
impl Worker {
    ...
    pub async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputListPeriodePerkuliahan) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        // Start transaction
