...
import {
    InstitutionReferenceControllerVarietyIndex,
    InstitutionReferenceControllerVarietyUpsert,
    InstitutionReferenceControllerVarietyDelete,
} from '~/controllers/institution/reference/InstitutionReferenceVarietyController';

export default function InstitutionReferenceVarietyPage() {
    const [items, setItems] = createSignal<InstitutionReferenceVariety[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<InstitutionReferenceVariety | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await InstitutionReferenceControllerVarietyIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as InstitutionReferenceVariety[];
            ...
            console.error('Error loading institution variety data:', error);
            toast.danger('Failed to load institution variety data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: InstitutionReferenceVariety) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: InstitutionReferenceVariety) => {
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await InstitutionReferenceControllerVarietyUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Institution variety reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create institution variety reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await InstitutionReferenceControllerVarietyUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Institution variety reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update institution variety reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await InstitutionReferenceControllerVarietyDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Institution variety reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete institution variety reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Institution Variety / Ragam Institusi
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for institution varieties, governance models, and institutional types.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by variety name (e.g. Negeri, Swasta, Kedinasan)..."
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No institution variety records found</p>
                        ...
                        <Show
                            ...
                            fallback={
                                <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                    No institution variety records found.
                                </div>
                            }
                        >
                            <For each={items()}>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Institution Variety Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new institution variety reference record.
                            </p>
                        </div>
                        <button
                            type="button"
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Institution Variety Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify variety reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
