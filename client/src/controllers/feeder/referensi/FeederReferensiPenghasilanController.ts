...
const path = 'feeder/referensi/penghasilan';
...
export async function FeederReferensiControllerPenghasilanIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiPenghasilan>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerPenghasilanIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerPenghasilanUpsert(
    form: {
        id?: string | null;
        id_penghasilan?: number | null;
        nama_penghasilan?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiPenghasilan }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_penghasilan: Number(form.id_penghasilan),
            nama_penghasilan: form.nama_penghasilan,
        };
...
export async function FeederReferensiControllerPenghasilanDelete(
    props: { id: string },
...
export async function FeederReferensiControllerPenghasilanList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_penghasilan || String(item.id),
        }));
        return {
            code: 200,
            message: items,
