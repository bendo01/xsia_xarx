...
export async function LocationVillageControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<LocationVillage>> {
    try {
        ...
        if (props.sub_district_id) params.append('sub_district_id', props.sub_district_id);
        ...
        console.error('Error in LocationVillageControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function LocationVillageControllerUpsert(
    form: {
        id?: string | null;
        code: string;
        name: string;
...
): Promise<{ is_error: boolean; message: string; data?: LocationVillage }> {
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update village.' : 'Failed to create village.'),
            };
        }

        return {
            is_error: false,
            message: isUpdate ? 'Village updated successfully.' : 'Village created successfully.',
            data: resJson,
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while saving village.',
        };
    }
}

export async function LocationVillageControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete village.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Village deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting village.',
        };
    }
}
...
export async function getVillageLists(sub_district_id?: string): Promise<{
    ...
    try {
        ...
        if (sub_district_id) params.append('sub_district_id', sub_district_id);
...
export const LocationVillageControllerList = getVillageLists;
