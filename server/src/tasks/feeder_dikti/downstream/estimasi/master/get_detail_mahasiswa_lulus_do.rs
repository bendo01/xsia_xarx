...
const TASK_NAME: &str = "EstimateDetailMahasiswaLulusDO";
const API_ACTION: &str = "GetDetailMahasiswaLulusDO";
...
pub struct EstimateDetailMahasiswaLulusDO;

impl EstimateDetailMahasiswaLulusDO {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputDetailMahasiswaLulusDO) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[ModelInputDetailMahasiswaLulusDO],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<ModelInputDetailMahasiswaLulusDO>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateDetailMahasiswaLulusDO {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetDetailMahasiswaLulusDO data from Feeder Dikti"
    }
