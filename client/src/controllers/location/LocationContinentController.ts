...
export async function LocationContinentControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<LocationContinent>> {
    try {
        ...
        console.error('Error in LocationContinentControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function LocationContinentControllerUpsert(
    form: { id?: string | null; code?: number | string | null; alphabet_code?: string | null; name: string; slug?: string | null },
): Promise<{ is_error: boolean; message: string; data?: LocationContinent }> {
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update continent.' : 'Failed to create continent.'),
            };
        }

        return {
            is_error: false,
            message: isUpdate ? 'Continent updated successfully.' : 'Continent created successfully.',
            data: resJson,
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while saving continent.',
        };
    }
}

export async function LocationContinentControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete continent.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Continent deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting continent.',
        };
    }
}

export async function LocationContinentControllerList(): Promise<{
    code: number;
