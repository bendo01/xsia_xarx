...
const TASK_NAME: &str = "EstimateGetJenisKeluar";
const API_ACTION: &str = "GetJenisKeluar";
...
pub struct EstimateGetJenisKeluar;

impl EstimateGetJenisKeluar {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisKeluarResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenis_keluar = record
            .id_jenis_keluar
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenis_keluar is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJenisKeluarResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenisKeluarResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenisKeluar {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenisKeluar data from Feeder Dikti"
    }
