...
const path = "person/reference/marital-status";
...
export async function PersonReferenceControllerMaritalStatusIndex(pagination: TypePaginationForm): Promise<ModelCommonReferencePaginationResponse> {
    try {
        ...
        console.error("Error fetching marital status reference:", error);
        return {
            pagination: {
                search: "",
                sort_by: "",
...
export async function PersonReferenceControllerMaritalStatusUpsert(data: TypeInputEntityReferenceForm): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully saved marital status reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            ...
            returned.message = responseData.message || responseData.brief || "Failed to save marital status reference.";
            if (responseData.errors) returned.errors = responseData.errors;
            return returned;
        }

        returned.message = isUpdate ? "Successfully updated marital status reference." : "Successfully created marital status reference.";
        return returned;
    } catch (error: any) {
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while saving marital status reference.";
        return returned;
    }
}

export async function PersonReferenceControllerMaritalStatusDelete(data: TypeInputEntityReferenceForm | { id?: string | null }): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully deleted marital status reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            returned.is_error = true;
            returned.code = response.status;
            returned.message = responseData.message || responseData.brief || "Failed to delete marital status reference.";
            return returned;
        }
        ...
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while deleting marital status reference.";
        return returned;
    }
}

export async function PersonReferenceControllerMaritalStatusList(): Promise<{
    code: number;
    ...
    try {
        ...
        if (!response.ok) {
            return {
                ...
                message: "Failed to fetch marital status list",
            };
        }
