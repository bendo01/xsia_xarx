...
const path = 'feeder/referensi/agama';
...
export async function FeederReferensiControllerAgamaIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiAgama>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerAgamaIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerAgamaUpsert(
    form: {
        id?: string | null;
        id_agama?: number | null;
        nama_agama?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiAgama }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_agama: Number(form.id_agama),
            nama_agama: form.nama_agama,
        };
...
export async function FeederReferensiControllerAgamaDelete(
    props: { id: string },
...
export async function FeederReferensiControllerAgamaList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_agama || String(item.id),
        }));
        return {
            code: 200,
            message: items,
