...
const TASK_NAME: &str = "EstimateListKurikulum";
...
pub struct EstimateListKurikulum;

impl EstimateListKurikulum {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputListKurikulum) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[ModelInputListKurikulum],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<ModelInputListKurikulum>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateListKurikulum {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetListKurikulum data from Feeder Dikti"
    }
