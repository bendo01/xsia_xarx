...
import type { PersonReferenceHairType } from '~/models/person/reference/HairType';
...
export default function PersonReferenceHairTypePage() {
    const [items, setItems] = createSignal<PersonReferenceHairType[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<PersonReferenceHairType | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await PersonReferenceControllerHairTypeIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as PersonReferenceHairType[];
            ...
            console.error('Error loading hair type data:', error);
            toast.danger('Failed to load hair type data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: PersonReferenceHairType) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: PersonReferenceHairType) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Hair type name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerHairTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Hair type reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create hair type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await PersonReferenceControllerHairTypeUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Hair type reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update hair type reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await PersonReferenceControllerHairTypeDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Hair type reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete hair type reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Hair Type / Jenis Rambut
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for hair types (Lurus, Ikal, Keriting, etc.).
                    </p>
                </div>
                <div class="mt-4 sm:mt-0 flex items-center gap-2 justify-end">
                    <button
                        ...
                        id="btn-add-hair-type"
                    >
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M5 12h14" />
                            <path d="M12 5v14" />
                        </svg>
                        <span>Add Hair Type</span>
                    </button>
                </div>
            </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by hair type (e.g. Lurus, Ikal, Keriting)..."
                            onInput={handleSearch}
                            id="input-search-hair-type"
                        />
                    </div>
                </div>
                        ...
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Hair Type Name</th>
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No hair type records found</p>
                                                        <p class="text-xs">Click "Add Hair Type" to add a new record.</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        }
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new hair type reference record.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g. STR, WAV, CRL, KNK..."
                        ...
                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Hair Type Name <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                ...
                                placeholder="e.g. Lurus, Ikal, Keriting..."
                                value={formData().name}
                        ...
                        <div class="flex justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            <button
                                ...
                                disabled={isSubmitting()}
                            >
                                {isSubmitting() ? 'Saving...' : 'Save Hair Type'}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify hair type reference attributes.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <form onSubmit={handleEditSubmit} class="space-y-4 pt-4">
                        ...
                        <div class="flex justify-end gap-3 pt-4 border-t border-neutral-200 dark:border-neutral-700">
                            ...
                            <button
                                ...
                                disabled={isSubmitting()}
                            >
                                {isSubmitting() ? 'Updating...' : 'Update Hair Type'}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">Delete Hair Type</h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">This action cannot be undone.</p>
                        </div>
                    </div>

                    <p class="text-sm text-neutral-600 dark:text-neutral-400 mb-6">
                        Are you sure you want to delete the hair type <strong class="text-neutral-900 dark:text-white">"{selectedItem()?.name}"</strong>?
                    </p>
