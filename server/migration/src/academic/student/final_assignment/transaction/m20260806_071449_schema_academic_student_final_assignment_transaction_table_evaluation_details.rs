...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_071449_schema_academic_student_final_assignment_transaction_table_evaluation_details"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_student_final_assignment_transaction"), Alias::new("evaluation_details")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("evaluation_summary_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("adviser_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("mark").float().default(Expr::cust("'0'::real")),
                    )
                    .col(
                        ColumnDef::new("grade_id").uuid(),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("academic_student_final_assignment_transaction_evaluation_detail")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
