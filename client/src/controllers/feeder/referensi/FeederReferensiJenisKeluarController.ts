...
const path = 'feeder/referensi/jenis-keluar';
...
export async function FeederReferensiControllerJenisKeluarIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenisKeluar>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenisKeluarIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenisKeluarUpsert(
    form: {
        id?: string | null;
        id_jenis_keluar?: string | null;
        jenis_keluar?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenisKeluar }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenis_keluar: form.id_jenis_keluar,
            jenis_keluar: form.jenis_keluar,
        };
...
export async function FeederReferensiControllerJenisKeluarDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenisKeluarList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.jenis_keluar || String(item.id),
        }));
        return {
            code: 200,
            message: items,
