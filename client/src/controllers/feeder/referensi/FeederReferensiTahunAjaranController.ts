...
const path = 'feeder/referensi/tahun-ajaran';
...
export async function FeederReferensiControllerTahunAjaranIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiTahunAjaran>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerTahunAjaranIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerTahunAjaranUpsert(
    form: {
        id?: string | null;
        id_tahun_ajaran?: string | null;
        nama_tahun_ajaran?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiTahunAjaran }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_tahun_ajaran: form.id_tahun_ajaran,
...
export async function FeederReferensiControllerTahunAjaranDelete(
    props: { id: string },
...
export async function FeederReferensiControllerTahunAjaranList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_tahun_ajaran || String(item.id),
        }));
        return {
            code: 200,
            message: items,
