...
const path = 'feeder/referensi/wilayah';
...
export async function FeederReferensiControllerWilayahIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiWilayah>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerWilayahIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerWilayahUpsert(
    form: {
        id?: string | null;
        id_wilayah?: string | null;
        nama_wilayah?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiWilayah }> {
    try {
...
export async function FeederReferensiControllerWilayahDelete(
    props: { id: string },
...
export async function FeederReferensiControllerWilayahList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_wilayah || String(item.id),
        }));
        return {
            code: 200,
            message: items,
