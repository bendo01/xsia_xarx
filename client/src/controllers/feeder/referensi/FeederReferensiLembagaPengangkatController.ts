...
const path = 'feeder/referensi/lembaga-pengangkat';
...
export async function FeederReferensiControllerLembagaPengangkatIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiLembagaPengangkat>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerLembagaPengangkatIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerLembagaPengangkatUpsert(
    form: {
        id?: string | null;
        id_lembaga_angkat?: number | null;
        nama_lembaga_angkat?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiLembagaPengangkat }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_lembaga_angkat: Number(form.id_lembaga_angkat),
            nama_lembaga_angkat: form.nama_lembaga_angkat,
        };
...
export async function FeederReferensiControllerLembagaPengangkatDelete(
    props: { id: string },
...
export async function FeederReferensiControllerLembagaPengangkatList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_lembaga_angkat || String(item.id),
        }));
        return {
            code: 200,
            message: items,
