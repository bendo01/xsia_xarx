...
const path = 'feeder/referensi/status-mahasiswa';
...
export async function FeederReferensiControllerStatusMahasiswaIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiStatusMahasiswa>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerStatusMahasiswaIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerStatusMahasiswaUpsert(
    form: {
        id?: string | null;
        id_status_mahasiswa?: string | null;
        nama_status_mahasiswa?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiStatusMahasiswa }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_status_mahasiswa: form.id_status_mahasiswa,
            nama_status_mahasiswa: form.nama_status_mahasiswa,
        };
...
export async function FeederReferensiControllerStatusMahasiswaDelete(
    props: { id: string },
...
export async function FeederReferensiControllerStatusMahasiswaList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_status_mahasiswa || String(item.id),
        }));
        return {
            code: 200,
            message: items,
