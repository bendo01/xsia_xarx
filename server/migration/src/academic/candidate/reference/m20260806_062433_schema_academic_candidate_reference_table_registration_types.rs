...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_062433_schema_academic_candidate_reference_table_registration_types"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_candidate_reference")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_candidate_reference"), Alias::new("registration_types")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v4()")),
                    )
                    .col(
