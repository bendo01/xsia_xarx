...
const path = "person/reference/identification_type";
...
export async function PersonReferenceControllerIdentificationTypeDelete(data: TypeInputEntityReferenceForm): Promise<UpsertDeleteMessage> {
    let returned = {
        is_error: false,
        code: 200,
        message: "success to upsert reference.",
...
export async function PersonReferenceControllerIdentificationTypeList(): Promise<{
    ...
    try {
        const response = await fetch(`${server_api_url}person/reference/identification_types/list`, {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
            return {
                ...
                message: "Gagal Mengambil Data Identification Type"
            };
        }
