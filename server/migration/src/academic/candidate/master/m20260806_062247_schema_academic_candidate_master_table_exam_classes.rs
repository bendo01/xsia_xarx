use sea_orm_migration::prelude::*;

pub struct Migration;

impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_062247_schema_academic_candidate_master_table_exam_classes"
    }
}

#[async_trait::async_trait]
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_candidate_master")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_candidate_master"), Alias::new("exam_classes")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("code").integer(),
                    )
                    .col(
                        ColumnDef::new("alphabet_code").string_len(255),
                    )
                    .col(
                        ColumnDef::new("name").string_len(255).not_null(),
                    )
                    .col(
                        ColumnDef::new("phase_id").uuid().default(Expr::cust("'00000000-0000-0000-0000-000000000000'::uuid")),
                    )
                    .col(
                        ColumnDef::new("start_date").date_time().not_null(),
                    )
                    .col(
                        ColumnDef::new("end_date").date_time(),
                    )
                    .col(
                        ColumnDef::new("start_time").time(),
                    )
                    .col(
                        ColumnDef::new("end_time").time(),
                    )
                    .col(
                        ColumnDef::new("capacity").integer().not_null().default(0),
                    )
                    .col(
                        ColumnDef::new("lms_category").integer().default(0),
                    )
                    .col(
                        ColumnDef::new("is_online").boolean().default(Expr::cust("false")),
                    )
                    .col(
                        ColumnDef::new("created_at").date_time().default(Expr::cust("now()")),
                    )
                    .col(
                        ColumnDef::new("updated_at").date_time().default(Expr::cust("now()")),
                    )
                    .col(
                        ColumnDef::new("deleted_at").date_time(),
                    )
                    .col(
                        ColumnDef::new("sync_at").date_time(),
                    )
                    .col(
                        ColumnDef::new("created_by").uuid().default(Expr::cust("'00000000-0000-0000-0000-000000000000'::uuid")),
                    )
                    .col(
                        ColumnDef::new("updated_by").uuid().default(Expr::cust("'00000000-0000-0000-0000-000000000000'::uuid")),
                    )
                    .primary_key(
                        Index::create()
                            .name("acm_exam_classes_pkey")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
            .await
    }

    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .drop_table(
                Table::drop()
                    .table((Alias::new("academic_candidate_master"), Alias::new("exam_classes")))
                    .if_exists()
                    .to_owned(),
            )
            .await
    }
}
