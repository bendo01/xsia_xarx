...
const TASK_NAME: &str = "EstimateGetAlatTransportasi";
const API_ACTION: &str = "GetAlatTransportasi";
...
pub struct EstimateGetAlatTransportasi;

impl EstimateGetAlatTransportasi {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetAlatTransportasiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetAlatTransportasiResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetAlatTransportasiResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetAlatTransportasi {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetAlatTransportasi data from Feeder Dikti"
    }
