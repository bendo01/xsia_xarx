...
const path = 'feeder/referensi/semester';
...
export async function FeederReferensiControllerSemesterIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiSemester>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerSemesterIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerSemesterUpsert(
    form: {
        id?: string | null;
        id_semester?: string | null;
        nama_semester?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiSemester }> {
    try {
...
export async function FeederReferensiControllerSemesterDelete(
    props: { id: string },
...
export async function FeederReferensiControllerSemesterList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_semester || String(item.id),
        }));
        return {
            code: 200,
            message: items,
