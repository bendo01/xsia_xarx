...
const TASK_NAME: &str = "EstimateListPerkuliahanMahasiswa";
const API_ACTION: &str = "GetListPerkuliahanMahasiswa";
...
pub struct EstimateListPerkuliahanMahasiswa;

impl EstimateListPerkuliahanMahasiswa {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputListPerkuliahanMahasiswa) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[ModelInputListPerkuliahanMahasiswa],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<ModelInputListPerkuliahanMahasiswa>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateListPerkuliahanMahasiswa {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetListPerkuliahanMahasiswa data from Feeder Dikti"
    }
