...
use crate::models::feeder::referensi::jenis_tinggal as jenis_tinggal;
use crate::tasks::feeder_dikti::downstream::feeder_request::{InputRequestData, RequestData};
use crate::tasks::Task;

// Configuration constants
const TASK_NAME: &str = "EstimateGetJenisTinggal";
const API_ACTION: &str = "GetJenisTinggal";
...
pub struct GetJenisTinggalResponse {
    pub id_jenis_tinggal: Option<String>,
    pub nama_jenis_tinggal: String,
}

pub struct EstimateGetJenisTinggal;

impl EstimateGetJenisTinggal {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenisTinggalResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenis_tinggal = record
            .id_jenis_tinggal
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenis_tinggal is missing".to_string()))?;

        let sync_time = Local::now().naive_local();

        let existing = jenis_tinggal::Entity::find()
            .filter(jenis_tinggal::Column::DeletedAt.is_null())
            .filter(jenis_tinggal::Column::IdJenisTinggal.eq(id_jenis_tinggal.clone()))
            .one(txn)
            .await?;

        let action = if let Some(existing_record) = existing {
            let mut active: jenis_tinggal::ActiveModel = existing_record.into_active_model();

            // Update fields that are present in GetJenisTinggalResponse
            active.nama_jenis_tinggal = Set(Some(record.nama_jenis_tinggal.clone()));
            ...
            let new_record = jenis_tinggal::ActiveModel {
                id: Set(pk_id),
                id_jenis_tinggal: Set(Some(id_jenis_tinggal)),
                nama_jenis_tinggal: Set(Some(record.nama_jenis_tinggal.clone())),
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJenisTinggalResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenisTinggalResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenisTinggal {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenisTinggal data from Feeder Dikti"
    }
