...
import type { BuildingReferenceCategory } from '~/models/building/reference/Category';
import type { TypeInputEntityReferenceForm } from '~/lib/types';
import {
    BuildingReferenceControllerCategoryIndex,
    BuildingReferenceControllerCategoryUpsert,
    BuildingReferenceControllerCategoryDelete,
} from '~/controllers/building/reference/BuildingReferenceCategoryController';

export default function BuildingReferenceCategoryPage() {
    const [items, setItems] = createSignal<BuildingReferenceCategory[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<BuildingReferenceCategory | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await BuildingReferenceControllerCategoryIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as BuildingReferenceCategory[];
            ...
            console.error('Error loading building category data:', error);
            toast.danger('Failed to load building category data from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: BuildingReferenceCategory) => {
        setFormData({
            id: item.id,
            code: item.code ?? '',
    ...
    const openDeleteModal = (item: BuildingReferenceCategory) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.name || currentForm.name.trim() === '') {
            errors.name = 'Building category name is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await BuildingReferenceControllerCategoryUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Building category reference created successfully!', 5000);
                closeCreateModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to create building category reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleEditSubmit = async (e: Event) => {
        ...
        try {
            const res = await BuildingReferenceControllerCategoryUpsert(formData());
            if (!res.is_error) {
                toast.success(res.message || 'Building category reference updated successfully!', 5000);
                closeEditModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to update building category reference.', 5000);
            }
        } catch (error: any) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await BuildingReferenceControllerCategoryDelete({ id: item.id });
            if (!res.is_error) {
                toast.success(res.message || 'Building category reference deleted successfully!', 5000);
                closeDeleteModal();
                fetchData();
            } else {
                toast.danger(res.message || 'Failed to delete building category reference.', 5000);
            }
        } catch (error: any) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">
                        Building Category / Kategori Bangunan
                    </h1>
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Manage master reference categories for facility and building infrastructure classifications.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search by category name (e.g. Gedung Kuliah, Gedung Rektorat)..."
                            onInput={handleSearch}
                            id="input-search-building-category"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-building-category"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-building-category"
                        >
                                            ...
                                            <tr>
                                                <td colspan="5" class="px-6 py-12 text-center text-neutral-500 dark:text-neutral-400">
                                                    <div class="flex flex-col items-center justify-center space-y-2">
                                                        ...
                                                        <p class="text-sm font-medium">No building category records found</p>
                        ...
                        <Show
                            ...
                            fallback={
                                <div class="p-8 text-center text-neutral-500 dark:text-neutral-400">
                                    No building category records found.
                                </div>
                            }
                        >
                            <For each={items()}>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Building Category Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Create a new building category reference record.
                            </p>
                        </div>
                        <button
                            type="button"
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Building Category Reference
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                                Modify building category reference properties.
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div class="flex items-center gap-3 pb-3 border-b border-neutral-200 dark:border-neutral-700">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">
                                This category record will be removed.
                            </p>
                        </div>
                    </div>
