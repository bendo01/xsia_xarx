...
export async function AcademicGeneralReferenceControllerAcademicYearIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicGeneralReferenceAcademicYear>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/general/reference/academic-years?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicGeneralReferenceControllerAcademicYearIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicGeneralReferenceControllerAcademicYearUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicGeneralReferenceAcademicYear }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/general/reference/academic-years/${form.id}`
            : `${getBaseUrl()}/academic/general/reference/academic-years`;
...
export async function AcademicGeneralReferenceControllerAcademicYearDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/general/reference/academic-years/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
