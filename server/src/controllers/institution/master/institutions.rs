...
use crate::dtos::institution::master::institutions::{
    CreateInstitutionRequest, InstitutionQuery, PaginatedInstitutionResponse,
    InstitutionResponse, UpdateInstitutionRequest,
};
...
pub async fn create_institution(
    ...
    let payload: CreateInstitutionRequest = req
        .parse_json()
        .await
...
pub async fn update_institution(
    ...
    let payload: UpdateInstitutionRequest = req
        .parse_json()
        .await
        .map_err(|e| StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e)))?;
    ...
    if let Some(val) = payload.alphabet_code {
        active_model.alphabet_code = Set(Some(val));
    }
    if let Some(val) = payload.is_active {
        active_model.is_active = Set(val);
    }
    if let Some(val) = payload.variety_id {
        active_model.variety_id = Set(val);
    }
    if let Some(val) = payload.category_id {
        active_model.category_id = Set(val);
    }
    if let Some(val) = payload.country_id {
        active_model.country_id = Set(val);
    }
    if let Some(val) = payload.parent_id {
        active_model.parent_id = Set(Some(val));
    }
    if let Some(val) = payload.feeder_id {
        active_model.feeder_id = Set(Some(val));
    }
    if let Some(val) = payload.academic_year_id {
        active_model.academic_year_id = Set(Some(val));
    }
...
fn to_response(item: entity_mod::Model) -> InstitutionResponse {
    InstitutionResponse {
        id: item.id,
        code: item.code,
        name: item.name,
