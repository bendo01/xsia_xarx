...
const path = 'feeder/referensi/pembiayaan';
...
export async function FeederReferensiControllerPembiayaanIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiPembiayaan>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerPembiayaanIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerPembiayaanUpsert(
    form: {
        id?: string | null;
        id_pembiayaan?: number | null;
        nama_pembiayaan?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiPembiayaan }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_pembiayaan: Number(form.id_pembiayaan),
            nama_pembiayaan: form.nama_pembiayaan,
        };
...
export async function FeederReferensiControllerPembiayaanDelete(
    props: { id: string },
...
export async function FeederReferensiControllerPembiayaanList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_pembiayaan || String(item.id),
        }));
        return {
            code: 200,
            message: items,
