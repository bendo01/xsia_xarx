...
const API_ACTION: &str = "GetStatusKeaktifanPegawai";
...
impl EstimateGetStatusKeaktifanPegawai {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetStatusKeaktifanPegawaiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_status_aktif = record
            .id_status_aktif
            .clone()
            .ok_or_else(|| DbErr::Custom("id_status_aktif is missing".to_string()))?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetStatusKeaktifanPegawaiResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetStatusKeaktifanPegawai {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetStatusKeaktifanPegawai data from Feeder Dikti"
    }
