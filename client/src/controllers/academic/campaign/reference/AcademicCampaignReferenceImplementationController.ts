...
export async function AcademicCampaignReferenceControllerImplementationIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCampaignReferenceImplementation>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/implementations?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCampaignReferenceControllerImplementationIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCampaignReferenceControllerImplementationUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCampaignReferenceImplementation }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/campaign/reference/implementations/${form.id}`
            : `${getBaseUrl()}/academic/campaign/reference/implementations`;
...
export async function AcademicCampaignReferenceControllerImplementationDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/campaign/reference/implementations/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
