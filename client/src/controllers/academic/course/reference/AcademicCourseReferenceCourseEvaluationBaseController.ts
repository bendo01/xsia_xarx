...
export async function AcademicCourseReferenceControllerCourseEvaluationBaseIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCourseReferenceCourseEvaluationBase>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/course-evaluation-bases?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCourseReferenceControllerCourseEvaluationBaseIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCourseReferenceControllerCourseEvaluationBaseUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCourseReferenceCourseEvaluationBase }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/course/reference/course-evaluation-bases/${form.id}`
            : `${getBaseUrl()}/academic/course/reference/course-evaluation-bases`;
...
export async function AcademicCourseReferenceControllerCourseEvaluationBaseDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/course/reference/course-evaluation-bases/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
