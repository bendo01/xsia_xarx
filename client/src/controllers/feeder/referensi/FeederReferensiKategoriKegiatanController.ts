...
const path = 'feeder/referensi/kategori-kegiatan';
...
export async function FeederReferensiControllerKategoriKegiatanIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiKategoriKegiatan>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerKategoriKegiatanIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerKategoriKegiatanUpsert(
    form: {
        id?: string | null;
        id_kategori_kegiatan?: number | null;
        nama_kategori_kegiatan?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiKategoriKegiatan }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_kategori_kegiatan: Number(form.id_kategori_kegiatan),
            nama_kategori_kegiatan: form.nama_kategori_kegiatan,
        };
...
export async function FeederReferensiControllerKategoriKegiatanDelete(
    props: { id: string },
...
export async function FeederReferensiControllerKategoriKegiatanList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_kategori_kegiatan || String(item.id),
        }));
        return {
            code: 200,
            message: items,
