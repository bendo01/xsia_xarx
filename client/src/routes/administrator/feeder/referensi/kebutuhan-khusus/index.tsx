...
import type { FeederReferensiKebutuhanKhusus } from '~/models/feeder/referensi/KebutuhanKhusus';
import {
    FeederReferensiControllerKebutuhanKhususIndex,
    FeederReferensiControllerKebutuhanKhususUpsert,
    FeederReferensiControllerKebutuhanKhususDelete,
} from '~/controllers/feeder/referensi/FeederReferensiKebutuhanKhususController';

export default function FeederReferensiKebutuhanKhususPage() {
    const [items, setItems] = createSignal<FeederReferensiKebutuhanKhusus[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiKebutuhanKhusus | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerKebutuhanKhususIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiKebutuhanKhusus[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_kebutuhan_khusus || '').toLowerCase();
                    const bVal = String(b.nama_kebutuhan_khusus || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi kebutuhan khusus from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiKebutuhanKhusus) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiKebutuhanKhusus) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_kebutuhan_khusus || currentForm.nama_kebutuhan_khusus.trim() === '') {
            errors.nama_kebutuhan_khusus = 'Nama Kebutuhan Khusus is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerKebutuhanKhususUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerKebutuhanKhususDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi disabilitas dan kebutuhan khusus PDDIKTI Feeder.
                    </p>
                </div>
                ...
                <div class="w-full md:w-2/3">
                    ...
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            id="input-search-kebutuhan-khusus"
                        />
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-sort-kebutuhan-khusus"
                        >
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-kebutuhan-khusus"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Kebutuhan Khusus</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Kebutuhan Khusus</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_kebutuhan_khusus}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_kebutuhan_khusus}</h3>
                    ...
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Add Referensi Kebutuhan Khusus
                            </h2>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                onInput={(e) => setFormData({ ...formData(), id_kebutuhan_khusus: e.currentTarget.value })}
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Nama Kebutuhan Khusus <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_kebutuhan_khusus ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_kebutuhan_khusus}
                                onInput={(e) => setFormData({ ...formData(), nama_kebutuhan_khusus: e.currentTarget.value })}
                            />
                            {formErrors().nama_kebutuhan_khusus && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_kebutuhan_khusus}</p>
                            )}
                        </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    <div class="flex items-center justify-between pb-4 border-b border-neutral-200 dark:border-neutral-700">
                        <div>
                            <h2 class="text-lg font-bold text-neutral-900 dark:text-white">
                                Edit Referensi Kebutuhan Khusus
                            </h2>
                            <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.nama_kebutuhan_khusus}
                    </p>
