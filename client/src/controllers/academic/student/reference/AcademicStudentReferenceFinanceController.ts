...
export async function AcademicStudentReferenceControllerFinanceIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicStudentReferenceFinance>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/student/reference/finances?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicStudentReferenceControllerFinanceIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicStudentReferenceControllerFinanceUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicStudentReferenceFinance }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/student/reference/finances/${form.id}`
            : `${getBaseUrl()}/academic/student/reference/finances`;
...
export async function AcademicStudentReferenceControllerFinanceDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/student/reference/finances/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
...
export async function getFinanceLists(): Promise<{
    ...
    try {
        const response = await fetch(`${getBaseUrl()}/academic/student/reference/finances/list`, {
            method: 'GET',
            headers: {
