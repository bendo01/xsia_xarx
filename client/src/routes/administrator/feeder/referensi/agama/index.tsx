...
import type { FeederReferensiAgama } from '~/models/feeder/referensi/Agama';
import {
    FeederReferensiControllerAgamaIndex,
    FeederReferensiControllerAgamaUpsert,
    FeederReferensiControllerAgamaDelete,
} from '~/controllers/feeder/referensi/FeederReferensiAgamaController';

export default function FeederReferensiAgamaPage() {
    const [items, setItems] = createSignal<FeederReferensiAgama[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiAgama | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerAgamaIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiAgama[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_agama || '').toLowerCase();
                    const bVal = String(b.nama_agama || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi agama from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiAgama) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiAgama) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_agama || currentForm.nama_agama.trim() === '') {
            errors.nama_agama = 'Nama Agama is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerAgamaUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerAgamaDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi data agama PDDIKTI Feeder.
                    </p>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Agama</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Agama</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_agama}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_agama}</h3>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                onInput={(e) => setFormData({ ...formData(), id_agama: e.currentTarget.value })}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_agama ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_agama}
                                onInput={(e) => setFormData({ ...formData(), nama_agama: e.currentTarget.value })}
                            />
                            {formErrors().nama_agama && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_agama}</p>
                            )}
                        </div>
