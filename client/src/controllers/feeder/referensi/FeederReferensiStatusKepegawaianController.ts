...
const path = 'feeder/referensi/status-kepegawaian';
...
export async function FeederReferensiControllerStatusKepegawaianIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiStatusKepegawaian>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerStatusKepegawaianIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerStatusKepegawaianUpsert(
    form: {
        id?: string | null;
        id_status_pegawai?: number | null;
        nama_status_pegawai?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiStatusKepegawaian }> {
    try {
        ...
        const payload: Record<string, any> = {
            ...
            nama_status_pegawai: form.nama_status_pegawai,
        };
...
export async function FeederReferensiControllerStatusKepegawaianDelete(
    props: { id: string },
...
export async function FeederReferensiControllerStatusKepegawaianList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_status_pegawai || String(item.id),
        }));
        return {
            code: 200,
            message: items,
