...
use crate::dtos::feeder::referensi::tahun_ajaran::{
    CreateTahunAjaranRequest, TahunAjaranQuery, TahunAjaranResponse, PaginatedTahunAjaranResponse,
...
pub async fn list_tahun_ajaran(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<PaginatedTahunAjaranResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        StatusError::internal_server_error().brief("Database connection missing")
    })?;

    let query: TahunAjaranQuery = req.parse_queries().unwrap_or_default();
    ...
    let data = items.into_iter().map(|item| TahunAjaranResponse {
            id: item.id,
    ...
    Ok(Json(PaginatedTahunAjaranResponse {
        data,
        total,
        page,
        page_size,
...
#[endpoint(tags("Feeder - Referensi - TahunAjaran"), status_codes(200, 400, 404, 500))]
pub async fn get_tahun_ajaran(
    req: &mut Request,
    depot: &mut Depot,
) -> Result<Json<TahunAjaranResponse>, StatusError> {
    let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        StatusError::internal_server_error().brief("Database connection missing")
    })?;
    ...
    let item = entity_mod::Entity::find_by_id(id)
        ...
        .ok_or_else(|| StatusError::not_found().brief("TahunAjaran not found"))?;

    Ok(Json(TahunAjaranResponse {
            id: item.id,
            id_tahun_ajaran: item.id_tahun_ajaran,
            nama_tahun_ajaran: item.nama_tahun_ajaran,
            a_periode_aktif: item.a_periode_aktif,
...
}#[endpoint(tags("Feeder - Referensi - TahunAjaran"), status_codes(200, 400, 500))]
pub async fn create_tahun_ajaran(
        req: &mut Request,
        depot: &mut Depot,
) -> Result<Json<TahunAjaranResponse>, StatusError> {
        let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        ...
        let payload: CreateTahunAjaranRequest = req.parse_json().await.map_err(|e| {
...
#[endpoint(tags("Feeder - Referensi - TahunAjaran"), status_codes(200, 400, 404, 500))]
pub async fn update_tahun_ajaran(
        req: &mut Request,
        depot: &mut Depot,
) -> Result<Json<TahunAjaranResponse>, StatusError> {
        let db = depot.get_typed::<DatabaseConnection>().map_err(|_| {
        ...
        let payload: UpdateTahunAjaranRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
...
#[endpoint(tags("Feeder - Referensi - TahunAjaran"), status_codes(200, 400, 404, 500))]
pub async fn delete_tahun_ajaran(
        req: &mut Request,
        depot: &mut Depot,
        ...
        Ok(Json(MessageResponse {
            message: "TahunAjaran deleted successfully".to_string(),
        }))
}
