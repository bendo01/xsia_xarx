...
const path = 'feeder/referensi/kebutuhan-khusus';
...
export async function FeederReferensiControllerKebutuhanKhususIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiKebutuhanKhusus>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerKebutuhanKhususIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerKebutuhanKhususUpsert(
    form: {
        id?: string | null;
        id_kebutuhan_khusus?: number | null;
        nama_kebutuhan_khusus?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiKebutuhanKhusus }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_kebutuhan_khusus: Number(form.id_kebutuhan_khusus),
            nama_kebutuhan_khusus: form.nama_kebutuhan_khusus,
        };
...
export async function FeederReferensiControllerKebutuhanKhususDelete(
    props: { id: string },
...
export async function FeederReferensiControllerKebutuhanKhususList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_kebutuhan_khusus || String(item.id),
        }));
        return {
            code: 200,
            message: items,
