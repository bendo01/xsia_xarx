...
pub struct WorkerArgs {
    pub records: Vec<GetAllProdiResponse>,
}
...
pub async fn start_worker(
    ...
    let worker = WorkerBuilder::new("xsia-xarx:feeder_dikti:downstream:master:upsert:get_all_prodi")
        .data(db)
        .backend(storage)
        .build_fn(handle_job);
...
impl Worker {
    ...
    pub async fn upsert_record(txn: &DatabaseTransaction, record: &GetAllProdiResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_prodi = record
            .id_prodi
