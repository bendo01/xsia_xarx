...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260808_034546_schema_feeder_referensi_table_jenis_prestasi"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("feeder_referensi"), Alias::new("jenis_prestasi")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("id_jenis_prestasi").integer(),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("feeder_referensi_jenis_prestasi_pkey")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
