use sea_orm_migration::{prelude::*, schema::*};

pub struct Migration;

impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_071116_schema_academic_student_campaign_table_detail_activity_evaluation_components"
    }
}

#[async_trait::async_trait]
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_student_campaign")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_student_campaign"), Alias::new("detail_activity_evaluation_components")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("name").text(),
                    )
                    .col(
                        ColumnDef::new("detail_activity_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("course_evaluation_planning_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("mark").float().default(Expr::cust("'0'::real")),
                    )
                    .col(
                        ColumnDef::new("percentage").float().default(Expr::cust("'0'::real")),
                    )
                    .col(
                        ColumnDef::new("total").float().default(Expr::cust("'0'::real")),
                    )
                    .col(
                        ColumnDef::new("created_at").date_time().default(Expr::cust("now()")),
                    )
                    .col(
                        ColumnDef::new("updated_at").date_time().default(Expr::cust("now()")),
                    )
                    .col(
                        ColumnDef::new("deleted_at").date_time(),
                    )
                    .col(
                        ColumnDef::new("sync_at").date_time(),
                    )
                    .col(
                        ColumnDef::new("created_by").uuid().default(Expr::cust("'00000000-0000-0000-0000-000000000000'::uuid")),
                    )
                    .col(
                        ColumnDef::new("updated_by").uuid().default(Expr::cust("'00000000-0000-0000-0000-000000000000'::uuid")),
                    )
                    .primary_key(
                        Index::create()
                            .name("academic_student_campaign_detail_activity_evaluation_components")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
            .await
    }

    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .drop_table(
                Table::drop()
                    .table((Alias::new("academic_student_campaign"), Alias::new("detail_activity_evaluation_components")))
                    .if_exists()
                    .to_owned(),
            )
            .await
    }
}
