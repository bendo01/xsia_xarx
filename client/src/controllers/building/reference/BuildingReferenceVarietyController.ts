...
const path = "building/reference/varieties";

export async function BuildingReferenceControllerVarietyIndex(pagination: TypePaginationForm): Promise<ModelCommonReferencePaginationResponse> {
    try {
        ...
        console.error("Error fetching building variety reference:", error);
        return {
            pagination: {
                search: "",
                sort_by: "",
...
export async function BuildingReferenceControllerVarietyUpsert(data: TypeInputEntityReferenceForm): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully saved building variety reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            ...
            returned.message = responseData.message || responseData.brief || "Failed to save building variety reference.";
            if (responseData.errors) returned.errors = responseData.errors;
            return returned;
        }

        returned.message = isUpdate ? "Successfully updated building variety reference." : "Successfully created building variety reference.";
        return returned;
    } catch (error: any) {
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while saving building variety reference.";
        return returned;
    }
}

export async function BuildingReferenceControllerVarietyDelete(data: TypeInputEntityReferenceForm | { id?: string | null }): Promise<UpsertDeleteMessage> {
    const returned: UpsertDeleteMessage = {
        is_error: false,
        code: 200,
        message: "Successfully deleted building variety reference.",
        errors: {},
    };
    ...
    try {
        ...
        if (!response.ok) {
            returned.is_error = true;
            returned.code = response.status;
            returned.message = responseData.message || responseData.brief || "Failed to delete building variety reference.";
            return returned;
        }
        ...
        returned.is_error = true;
        returned.code = 500;
        returned.message = error.message || "Network error while deleting building variety reference.";
        return returned;
    }
}

export async function BuildingReferenceControllerVarietyList(): Promise<{
    code: number;
    ...
    try {
        ...
        if (!response.ok) {
            return {
                ...
                message: "Failed to fetch building variety list",
            };
        }
