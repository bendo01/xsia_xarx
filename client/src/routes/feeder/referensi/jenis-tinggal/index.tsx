...
import type { FeederReferensiJenisTinggal } from '~/models/feeder/referensi/JenisTinggal';
import {
    FeederReferensiControllerJenisTinggalIndex,
    FeederReferensiControllerJenisTinggalUpsert,
    FeederReferensiControllerJenisTinggalDelete,
} from '~/controllers/feeder/referensi/FeederReferensiJenisTinggalController';

export default function FeederReferensiJenisTinggalPage() {
    const [items, setItems] = createSignal<FeederReferensiJenisTinggal[]>([]);
    ...
    const [selectedItem, setSelectedItem] = createSignal<FeederReferensiJenisTinggal | null>(null);

    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [field, dir] = sortParam().split('-');
            const response = await FeederReferensiControllerJenisTinggalIndex({
                page: currentPage(),
            ...
            if (response && response.data) {
                let data = response.data as unknown as FeederReferensiJenisTinggal[];
                const order = dir === 'desc' ? -1 : 1;
                data = [...data].sort((a, b) => {
                    const aVal = String(a.nama_jenis_tinggal || '').toLowerCase();
                    const bVal = String(b.nama_jenis_tinggal || '').toLowerCase();
            ...
            toast.danger('Failed to load referensi jenis tinggal from server.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openEditModal = (item: FeederReferensiJenisTinggal) => {
        setFormData({
            id: item.id,
    ...
    const openDeleteModal = (item: FeederReferensiJenisTinggal) => {
    ...
    const validateForm = (): boolean => {
        ...
        if (!currentForm.nama_jenis_tinggal || currentForm.nama_jenis_tinggal.trim() === '') {
            errors.nama_jenis_tinggal = 'Nama Jenis Tinggal is required.';
        }
    ...
    const handleCreateSubmit = async (e: Event) => {
        ...
        try {
            const res = await FeederReferensiControllerJenisTinggalUpsert(formData());
            if (!res.is_error) {
    ...
    const handleDeleteSubmit = async () => {
        ...
        setIsSubmitting(true);
        try {
            const res = await FeederReferensiControllerJenisTinggalDelete({ id: item.id });
            if (!res.is_error) {
            ...
            <div class="sm:flex sm:items-center sm:justify-between mb-4 px-3 pt-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                        Master referensi jenis tempat tinggal mahasiswa PDDIKTI Feeder.
                    </p>
                </div>
                ...
                <div class="w-full md:w-1/3 flex gap-2">
                    ...
                    <div class="w-1/2">
                        ...
                        <select
                            ...
                            id="select-per-page-jenis-tinggal"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left whitespace-nowrap">
                            <thead class="text-xs text-neutral-600 uppercase bg-neutral-100 dark:bg-neutral-800 dark:text-neutral-300 border-b border-neutral-200 dark:border-neutral-700">
                                <tr>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">ID Jenis Tinggal</th>
                                    <th scope="col" class="px-6 py-3.5 font-semibold tracking-wider">Nama Jenis Tinggal</th>
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors">
                                                    ...
                                                    <td class="px-6 py-4">
                                                        <div class="font-medium text-neutral-900 dark:text-white">{item.nama_jenis_tinggal}</div>
                            ...
                            <For each={items()}>
                                {(item) => (
                                    <div class="p-4 space-y-2">
                                        ...
                                        <div>
                                            <h3 class="font-medium text-neutral-900 dark:text-white">{item.nama_jenis_tinggal}</h3>
                    ...
                    <form onSubmit={handleCreateSubmit} class="space-y-4 pt-4">
                        <div>
                            ...
                            <input
                                ...
                                onInput={(e) => setFormData({ ...formData(), id_jenis_tinggal: e.currentTarget.value })}
                            />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
                                Nama Jenis Tinggal <span class="text-red-500">*</span>
                            </label>
                            <input
                                type="text"
                                class={`block w-full p-2.5 text-sm text-neutral-900 border rounded-none bg-white focus:ring-blue-500 focus:border-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:text-white transition-colors ${
                                    formErrors().nama_jenis_tinggal ? 'border-red-500 dark:border-red-500' : 'border-neutral-300'
                                }`}
                                placeholder="Enter name..."
                                value={formData().nama_jenis_tinggal}
                                onInput={(e) => setFormData({ ...formData(), nama_jenis_tinggal: e.currentTarget.value })}
                            />
                            {formErrors().nama_jenis_tinggal && (
                                <p class="text-xs text-red-500 mt-1">{formErrors().nama_jenis_tinggal}</p>
                            )}
                        </div>
            ...
            >
                <div class="p-6 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
                    ...
                    <p class="text-sm text-neutral-600 dark:text-neutral-300 bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-none border border-neutral-200 dark:border-neutral-700 mb-4 font-mono text-xs">
                        {selectedItem()?.nama_jenis_tinggal}
                    </p>
