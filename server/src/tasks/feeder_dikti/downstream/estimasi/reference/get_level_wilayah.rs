...
const TASK_NAME: &str = "EstimateGetLevelWilayah";
const API_ACTION: &str = "GetLevelWilayah";
...
pub struct EstimateGetLevelWilayah;

impl EstimateGetLevelWilayah {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetLevelWilayahResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_level_wilayah = record
            .id_level_wilayah
            .ok_or_else(|| DbErr::Custom("id_level_wilayah is missing".to_string()))?
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetLevelWilayahResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetLevelWilayahResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetLevelWilayah {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetLevelWilayah data from Feeder Dikti"
    }
