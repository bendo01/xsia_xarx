...
const path = 'feeder/referensi/jenjang-pendidikan';
...
export async function FeederReferensiControllerJenjangPendidikanIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenjangPendidikan>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenjangPendidikanIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenjangPendidikanUpsert(
    form: {
        id?: string | null;
        id_jenjang_didik?: number | null;
        nama_jenjang_didik?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenjangPendidikan }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenjang_didik: Number(form.id_jenjang_didik),
            nama_jenjang_didik: form.nama_jenjang_didik,
        };
...
export async function FeederReferensiControllerJenjangPendidikanDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenjangPendidikanList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jenjang_didik || String(item.id),
        }));
        return {
            code: 200,
            message: items,
