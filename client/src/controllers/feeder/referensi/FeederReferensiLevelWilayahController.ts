...
const path = 'feeder/referensi/level-wilayah';
...
export async function FeederReferensiControllerLevelWilayahIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiLevelWilayah>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerLevelWilayahIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerLevelWilayahUpsert(
    form: {
        id?: string | null;
        id_level_wilayah?: number | null;
        nama_level_wilayah?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiLevelWilayah }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_level_wilayah: Number(form.id_level_wilayah),
            nama_level_wilayah: form.nama_level_wilayah,
        };
...
export async function FeederReferensiControllerLevelWilayahDelete(
    props: { id: string },
...
export async function FeederReferensiControllerLevelWilayahList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_level_wilayah || String(item.id),
        }));
        return {
            code: 200,
            message: items,
