...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_071300_schema_academic_student_final_assignment_reference_table_adviser_categories"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("CREATE SCHEMA IF NOT EXISTS academic_student_final_assignment_reference")
            .await?;

        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_student_final_assignment_reference"), Alias::new("adviser_categories")))
                    .if_not_exists()
                    .col(
                        ColumnDef::new("id").uuid().not_null().default(Expr::cust("public.uuid_generate_v7()")),
                    )
                    .col(
                        ColumnDef::new("code").integer().not_null(),
                    )
                    .col(
                    ...
                    .primary_key(
                        Index::create()
                            .name("academic_student_final_assignment_reference_adviser_categories_")
                            .col(Alias::new("id")),
                    )
                    .to_owned(),
            )
