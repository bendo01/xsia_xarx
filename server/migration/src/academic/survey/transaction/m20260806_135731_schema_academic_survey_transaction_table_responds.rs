...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260806_135731_schema_academic_survey_transaction_table_responds"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager
            .create_table(
                Table::create()
                    .table((Alias::new("academic_survey_transaction"), Alias::new("responds")))
                    .if_not_exists()
                    .col(
                    ...
                    .col(
                        ColumnDef::new("conduct_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("bundle_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("question_id").uuid().not_null(),
                    )
                    .col(
                        ColumnDef::new("answer_id").uuid(),
                    )
                    .col(
