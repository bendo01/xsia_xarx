...
use crate::models::location::continents as entity_mod;
...
pub async fn list_continents(
    ...
    let data = items.into_iter().map(|item| ContinentResponse {
            id: item.id,
            code: item.code,
...
pub async fn create_continent(
        ...
        let payload: CreateContinentRequest = req.parse_json().await.map_err(|e| {
...
pub async fn update_continent(
        ...
        let payload: UpdateContinentRequest = req.parse_json().await.map_err(|e| {
            StatusError::bad_request().brief(format!("Invalid JSON payload: {}", e))
        })?;
