...
const TASK_NAME: &str = "EstimateGetJenjangPendidikan";
const API_ACTION: &str = "GetJenjangPendidikan";
...
pub struct EstimateGetJenjangPendidikan;

impl EstimateGetJenjangPendidikan {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetJenjangPendidikanResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_jenjang_didik = record
            .id_jenjang_didik
            .clone()
            .ok_or_else(|| DbErr::Custom("id_jenjang_didik is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetJenjangPendidikanResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetJenjangPendidikanResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetJenjangPendidikan {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetJenjangPendidikan data from Feeder Dikti"
    }
