...
use crate::models::feeder::referensi::status_mahasiswa as status_mahasiswa;
use crate::tasks::feeder_dikti::downstream::feeder_request::{InputRequestData, RequestData};
use crate::tasks::Task;

// Configuration constants
const TASK_NAME: &str = "EstimateGetStatusMahasiswa";
const API_ACTION: &str = "GetStatusMahasiswa";
...
pub struct GetStatusMahasiswaResponse {
    pub id_status_mahasiswa: Option<String>,
    pub nama_status_mahasiswa: Option<String>,
}

pub struct EstimateGetStatusMahasiswa;

impl EstimateGetStatusMahasiswa {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetStatusMahasiswaResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_status_mahasiswa = record
            .id_status_mahasiswa
            .clone()
            .ok_or_else(|| DbErr::Custom("id_status_mahasiswa is missing".to_string()))?;

        let sync_time = Local::now().naive_local();

        let existing = status_mahasiswa::Entity::find()
            .filter(status_mahasiswa::Column::DeletedAt.is_null())
            .filter(status_mahasiswa::Column::IdStatusMahasiswa.eq(&id_status_mahasiswa))
            .one(txn)
            .await?;

        let action = if let Some(existing_record) = existing {
            let mut active: status_mahasiswa::ActiveModel = existing_record.into_active_model();
            ...
            let new_record = status_mahasiswa::ActiveModel {
                id: Set(pk_id),
                id_status_mahasiswa: Set(Some(id_status_mahasiswa)),
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetStatusMahasiswaResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetStatusMahasiswaResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetStatusMahasiswa {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetStatusMahasiswa data from Feeder Dikti"
    }
