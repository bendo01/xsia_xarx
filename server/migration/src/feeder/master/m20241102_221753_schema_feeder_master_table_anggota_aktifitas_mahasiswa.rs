...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            "
            CREATE TABLE IF NOT EXISTS feeder_master.anggota_aktifitas_mahasiswa
            (
                ...
                CONSTRAINT feeder_master_anggota_aktifitas_mahasiswa_pkey PRIMARY KEY (id)
            )
            ",
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared("DROP TABLE IF EXISTS feeder_master.anggota_aktifitas_mahasiswa")
            .await?;
