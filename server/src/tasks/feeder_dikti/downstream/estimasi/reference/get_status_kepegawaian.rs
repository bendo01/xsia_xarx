...
const TASK_NAME: &str = "EstimateGetStatusKepegawaian";
const API_ACTION: &str = "GetStatusKepegawaian";
...
pub struct EstimateGetStatusKepegawaian;

impl EstimateGetStatusKepegawaian {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetStatusKepegawaianResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_status_pegawai = record
            .id_status_pegawai
            .clone()
            .ok_or_else(|| DbErr::Custom("id_status_pegawai is missing".to_string()))?;
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetStatusKepegawaianResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetStatusKepegawaianResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetStatusKepegawaian {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetStatusKepegawaian data from Feeder Dikti"
    }
