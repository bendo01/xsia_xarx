...
export async function AcademicCandidateReferenceControllerPhaseIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<AcademicCandidateReferencePhase>> {
    try {
        ...
        const res = await fetch(`${getBaseUrl()}/academic/candidate/reference/phases?${params.toString()}`);
        if (!res.ok) {
        ...
        console.error('Error in AcademicCandidateReferenceControllerPhaseIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function AcademicCandidateReferenceControllerPhaseUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: AcademicCandidateReferencePhase }> {
    try {
        const isUpdate = !!form.id;
        const url = isUpdate
            ? `${getBaseUrl()}/academic/candidate/reference/phases/${form.id}`
            : `${getBaseUrl()}/academic/candidate/reference/phases`;
...
export async function AcademicCandidateReferenceControllerPhaseDelete(
    props: { id: string },
): Promise<{ is_error: boolean; message: string }> {
    try {
        const res = await fetch(`${getBaseUrl()}/academic/candidate/reference/phases/${props.id}`, {
            method: 'DELETE',
        });
        const resJson = await res.json().catch(() => ({}));
        if (!res.ok) {
