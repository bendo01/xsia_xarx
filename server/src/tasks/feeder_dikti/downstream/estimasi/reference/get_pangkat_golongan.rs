...
const TASK_NAME: &str = "EstimateGetPangkatGolongan";
const API_ACTION: &str = "GetPangkatGolongan";
...
pub struct EstimateGetPangkatGolongan;

impl EstimateGetPangkatGolongan {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetPangkatGolonganResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_batch(
        db: &DatabaseConnection,
        records: &[GetPangkatGolonganResponse],
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let txn = db.begin().await?;
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetPangkatGolonganResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetPangkatGolongan {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetPangkatGolongan data from Feeder Dikti"
    }
