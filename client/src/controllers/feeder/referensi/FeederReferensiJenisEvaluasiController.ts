...
const path = 'feeder/referensi/jenis-evaluasi';
...
export async function FeederReferensiControllerJenisEvaluasiIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<FeederReferensiJenisEvaluasi>> {
    try {
        ...
        console.error('Error in FeederReferensiControllerJenisEvaluasiIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function FeederReferensiControllerJenisEvaluasiUpsert(
    form: {
        id?: string | null;
        id_jenis_evaluasi?: number | null;
        nama_jenis_evaluasi?: string | null;
    },
): Promise<{ is_error: boolean; message: string; data?: FeederReferensiJenisEvaluasi }> {
    try {
        ...
        const payload: Record<string, any> = {
            id_jenis_evaluasi: Number(form.id_jenis_evaluasi),
            nama_jenis_evaluasi: form.nama_jenis_evaluasi,
        };
...
export async function FeederReferensiControllerJenisEvaluasiDelete(
    props: { id: string },
...
export async function FeederReferensiControllerJenisEvaluasiList(): Promise<{
    code: number;
    ...
    try {
        ...
        const items: ModelSelectItem[] = list.map((item: any) => ({
            id: item.id,
            value: item.id,
            label: item.nama_jenis_evaluasi || String(item.id),
        }));
        return {
            code: 200,
            message: items,
