...
pub struct WorkerArgs {
    pub records: Vec<ModelInputDetailMatakuliah>,
}
...
pub async fn start_worker(
    ...
    let worker = WorkerBuilder::new("xsia-xarx:feeder_dikti:downstream:master:upsert:get_detail_matakuliah")
        .data(db)
        .backend(storage)
        .build_fn(handle_job);
...
impl Worker {
    ...
    pub async fn upsert_record(txn: &DatabaseTransaction, record: &ModelInputDetailMatakuliah) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
