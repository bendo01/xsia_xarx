...
use crate::models::feeder::referensi::negara as negara;
use crate::tasks::feeder_dikti::downstream::feeder_request::{InputRequestData, RequestData};
use crate::tasks::Task;

// Configuration constants
const TASK_NAME: &str = "EstimateGetNegara";
const API_ACTION: &str = "GetNegara";
...
pub struct GetNegaraResponse {
    pub id_negara: Option<String>,
    pub nama_negara: String,
}

pub struct EstimateGetNegara;

impl EstimateGetNegara {
    ...
    async fn upsert_record(txn: &DatabaseTransaction, record: &GetNegaraResponse) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let id_negara = record
            .id_negara
            .clone()
            .ok_or_else(|| DbErr::Custom("id_negara is missing".to_string()))?;

        let sync_time = Local::now().naive_local();

        let existing = negara::Entity::find()
            .filter(negara::Column::DeletedAt.is_null())
            .filter(negara::Column::IdNegara.eq(id_negara.clone()))
            .one(txn)
            .await?;

        let action = if let Some(existing_record) = existing {
            let mut active: negara::ActiveModel = existing_record.into_active_model();

            // Update fields that are present in GetNegaraResponse
            active.nama_negara = Set(Some(record.nama_negara.clone()));
            ...
            let new_record = negara::ActiveModel {
                id: Set(pk_id),
                id_negara: Set(Some(id_negara)),
                nama_negara: Set(Some(record.nama_negara.clone())),
    ...
    async fn fetch_and_process_page(
        ...
        let response = RequestData::get::<GetNegaraResponse>(
            InputRequestData {
...
#[async_trait]
impl Task for EstimateGetNegara {
    fn name(&self) -> &str {
        TASK_NAME
    }

    fn description(&self) -> &str {
        "Fetch and process GetNegara data from Feeder Dikti"
    }
