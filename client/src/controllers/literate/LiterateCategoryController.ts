...
export async function LiterateCategoryControllerIndex(
    props: TypePaginationForm,
): Promise<TypePaginationResponse<LiterateCategory>> {
    try {
        ...
        console.error('Error in LiterateCategoryControllerIndex:', e);
        return {
            pagination: {
                search: props.search || '',
                sort_by: props.sort_by || '',
...
export async function LiterateCategoryControllerUpsert(
    form: TypeInputEntityReferenceForm,
): Promise<{ is_error: boolean; message: string; data?: LiterateCategory }> {
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || (isUpdate ? 'Failed to update category.' : 'Failed to create category.'),
            };
        }
        ...
        return {
            is_error: true,
            message: error.message || 'Network error while saving category.',
        };
    }
}

export async function LiterateCategoryControllerDelete(
    props: { id: string },
    ...
    try {
        ...
        if (!res.ok) {
            return {
                is_error: true,
                message: resJson.message || resJson.brief || 'Failed to delete category.',
            };
        }
        return {
            is_error: false,
            message: resJson.message || 'Category deleted successfully.',
        };
    } catch (error: any) {
        return {
            is_error: true,
            message: error.message || 'Network error while deleting category.',
        };
    }
}

export async function LiterateCategoryControllerList(): Promise<{
    code: number;
